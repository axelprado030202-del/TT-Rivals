...
function expose(){
  ...
  Object.assign(window.__TT_PERF_V101,{motion:{mode,quality,fps:Math.round(fps),targetFps:targetFps(),canvasActive:visible?1:0,dpr:Number(dpr.toFixed(2)),reducedMotion:reduceMotion,saveData}});
}
function setQuality(next,reason='manual'){
  if(!QUALITY_ORDER.includes(next)||next===quality)return;
  ...
  window.dispatchEvent(new CustomEvent('tt:motion-quality',{detail:{quality,reason}}));
  expose();
}
...
window.__TT_MOTION_V101={getDiagnostics:()=>window.__TT_PERF_V101?.motion||{},setQuality};

...
async function contextFor(userId){
  const [ratingRes,streakRes,missionRes]=await Promise.all([
    supabase.from('ratings').select('rating').eq('user_id',userId).eq('modality','individual').maybeSingle().catch(()=>({data:null})),
    supabase.rpc('get_my_streak_status_v101').catch(()=>({data:null})),
    supabase.rpc('get_my_daily_missions_v101').catch(()=>({data:null}))
  ]);
  return {

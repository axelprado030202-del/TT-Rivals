import {APP_VERSION,APP_BUILD,APP_CACHE_NAME} from './version.js';

const TT_SW_URL_V60=`./service-worker.js?v=${encodeURIComponent(APP_VERSION)}`;
...
async function requestInstallV573(){
  ...
  alert('Chrome todavía no habilitó el instalador. Abrí el menú ⋮ y buscá “Instalar app” o “Agregar a pantalla principal”.');
}
...
export async function setupPwaV573(){
  ...
  window.addEventListener('beforeinstallprompt',event=>{
    event.preventDefault();deferredInstallPromptV573=event;setInstallUiV573('ready');
  });
  window.addEventListener('appinstalled',()=>{
    deferredInstallPromptV573=null;setInstallUiV573('installed');
  });

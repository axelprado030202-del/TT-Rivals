...
function openToolV61(tool){
  ...
  if(tool==='video')loadVideoHistoryV611({silent:true}).catch(()=>{});
}
...
function renderVideoAnalysisV61(data,{root=$('#aiVideoResultV61'),historyRecord=null}={}){
  ...
  if(!root)return;
  root.classList.remove('hidden');
  const saved=!!normalized.__analysis_id;
  root.innerHTML=`<div class="ai-result-head-v61"><div><p class="muted-label">${historyRecord?'ANÁLISIS GUARDADO':'ANÁLISIS TT AI'}</p><h3>${esc(data.title||'Lectura del video')}</h3><p>${esc(data.summary||'')}</p></div><span>${saved?'GUARDADO · ':''}${esc(data.confidence||'orientativo')}</span></div>
    ...
    <div class="ai-result-actions-v61">
      <button class="primary" data-ai-video-training-v611 type="button">🏓 Crear entrenamiento desde esto</button>
      <button data-ai-video-journal-v611 type="button">▤ Guardar resumen en diario</button>
      ${historyRecord?`<button class="ai-danger-soft-v611" data-ai-video-delete-v611="${esc(historyRecord.id)}" type="button">Eliminar análisis</button>`:''}
    </div>`;
...
async function analyzeVideoV61(){
  ...
  try{
    ...
    let id=null;
    try{
      id=await saveVideoAnalysisV61(focus,file,frames,result.result||{});
    }catch(saveErr){
      console.warn('No se pudo guardar análisis de Video Lab:',saveErr);
    }
    ...
    if(lastVideoAnalysis){
      lastVideoAnalysis.__analysis_id=id;
      lastVideoAnalysis.__focus=focus;
      lastVideoAnalysis.__video_metadata=liveResult.__video_metadata;
    }
    if(id){
      setText(status,'Análisis completado y guardado automáticamente en Mis análisis ✓');
      loadVideoHistoryV611({silent:true}).catch(()=>{});
    }else{
      setText(status,'Análisis completado. No se pudo guardar en el historial, pero podés leerlo ahora.');
    }
  }catch(err){
    console.error('Video Lab V61.1:',err);
...
export function refreshAiV61(){
  ...
  if($('#aiWorkspaceV61')?.dataset.tool==='video')loadVideoHistoryV611({silent:true}).catch(()=>{});
}

...
let videoQuotaV62={limit:3,used:0,reserved:0,remaining:null,tickets:0,can_analyze:null,next_source:'weekly',reset_at:null};
...
function syncVideoAnalyzeButtonV62(){
  ...
  const hasQuota=videoQuotaV62.can_analyze===null
    ?(videoQuotaV62.remaining===null||Number(videoQuotaV62.remaining)>0||Number(videoQuotaV62.tickets)>0)
    :!!videoQuotaV62.can_analyze;
  button.disabled=!selectedVideoFile||engineState!=='online'||!hasQuota;
}
function renderVideoQuotaV62(quota=videoQuotaV62){
  const box=$('#aiVideoQuotaV62'),text=$('#aiVideoQuotaTextV62'),badge=$('#aiVideoQuotaBadgeV62'),ticketBadge=$('#aiVideoTicketsBadgeV63');
  ...
  const tickets=Math.max(0,Number(quota?.tickets)||0);
  const canAnalyze=quota?.can_analyze===undefined?(remaining>0||tickets>0):!!quota.can_analyze;
  const nextSource=quota?.next_source||(remaining>0?'weekly':tickets>0?'ticket':'none');
  videoQuotaV62={limit,used,reserved,remaining,tickets,can_analyze:canAnalyze,next_source:nextSource,reset_at:quota?.reset_at||null,last_usage_source:quota?.last_usage_source||null};
  badge.textContent=`${remaining} / ${limit}`;
  if(ticketBadge)ticketBadge.textContent=`🎟 ${tickets}`;
  box.classList.toggle('is-empty',!canAnalyze);box.classList.toggle('is-low',remaining===1&&tickets===0);box.classList.toggle('has-tickets-v63',remaining<=0&&tickets>0);
  if(remaining>0){
    text.textContent=`${remaining} uso${remaining===1?'':'s'} semanal${remaining===1?'':'es'} disponible${remaining===1?'':'s'} · ${tickets} ticket${tickets===1?'':'s'} acumulable${tickets===1?'':'s'} guardado${tickets===1?'':'s'} · se renueva ${quotaResetLabelV62(videoQuotaV62.reset_at)}`;
  }else if(tickets>0){
    text.textContent=`Cupo semanal agotado · tenés ${tickets} ticket${tickets===1?'':'s'} acumulable${tickets===1?'':'s'}. El próximo análisis usará 1 ticket.`;
  }else{
    text.textContent=`Límite semanal alcanzado y sin tickets · se renueva ${quotaResetLabelV62(videoQuotaV62.reset_at)}`;
  }
  syncVideoAnalyzeButtonV62();
}
async function loadVideoQuotaV62({silent=false}={}){
  const text=$('#aiVideoQuotaTextV62');
  if(!silent&&text)text.textContent='Comprobando usos semanales y tickets…';
  try{
    const payload=await invokeAiV61('video_quota',{});
    renderVideoQuotaV62(payload.quota||{});
    return videoQuotaV62;
  }catch(err){
    videoQuotaV62={limit:3,used:0,reserved:0,remaining:0,tickets:0,can_analyze:false,next_source:'none',reset_at:null};
    ...
    const ticketBadge=$('#aiVideoTicketsBadgeV63');if(ticketBadge)ticketBadge.textContent='🎟 —';
    syncVideoAnalyzeButtonV62();
    if(!silent)console.warn('Cupo Video Lab V63:',err);
    return videoQuotaV62;
  }
}
...
async function analyzeVideoV61(){
  ...
  if(!videoQuotaV62.can_analyze){
    setText(status,`Alcanzaste los ${videoQuotaV62.limit||3} usos semanales y no tenés tickets disponibles. Tu historial sigue disponible sin consumir usos.`);syncVideoAnalyzeButtonV62();return;
  }
  if(Number(videoQuotaV62.remaining)<=0&&Number(videoQuotaV62.tickets)>0){
    setText(status,`El cupo semanal está agotado. Este análisis utilizará 1 de tus ${videoQuotaV62.tickets} ticket${Number(videoQuotaV62.tickets)===1?'':'s'} acumulables.`);
  }
  ...
  try{
    ...
    if(id){const usedTicket=result.quota?.last_usage_source==='ticket';setText(status,`Análisis completado y guardado ✓ · semanal ${videoQuotaV62.remaining}/${videoQuotaV62.limit} · 🎟 ${videoQuotaV62.tickets}${usedTicket?' · se utilizó 1 ticket':''}`);loadVideoHistoryV611({silent:true}).catch(()=>{})}

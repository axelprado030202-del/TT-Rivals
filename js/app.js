...
import {getFrameCalibrationsV36,saveFrameCalibrationV36,subscribeVisualLiveV36,getPublicAdminFlagV37,getPublicAdminIdsV38} from './v36_live.js';
...
let frameCalibrationsV42=new Map(),liveFramePreviewV42=null,stopAvatarLiveV42=null;
let adminUserIdsV38=new Set(),universalPlayerObserverV38=null;
const AVATAR_BASE_DIAMETER_V42=68;
let adminFrameDraftV42=null;
...
/* ============================================================
   V42 — ÚNICO MOTOR DE AVATAR
   Foto fija + FrameStage cuadrado + PNG encima
   ============================================================ */

const DEFAULT_FRAME_GEOMETRY_V42={
  frame_scale:100,
  frame_x:50,
  frame_y:50
};

function getFrameGeometryV42(frameId){
  const row=frameCalibrationsV42.get(frameId)||{};
  return {
    frame_scale:Number(row.frame_scale??100),
    frame_x:Number(row.frame_x??50),
    frame_y:Number(row.frame_y??50)
  };
}

function frameGeometryForOwnV42(frameId){
  if(
    v35Flags?.is_test_admin &&
    liveFramePreviewV42 &&
    liveFramePreviewV42.enabled &&
    liveFramePreviewV42.frameId===frameId
  ){
    return {
      frame_scale:Number(liveFramePreviewV42.frame_scale),
      frame_x:Number(liveFramePreviewV42.frame_x),
      frame_y:Number(liveFramePreviewV42.frame_y)
    };
  }
  return getFrameGeometryV42(frameId);
}

function normalizeAvatarHostV42(host){
  if(!host)return null;

  // Quita exclusivamente clases heredadas del subsistema de marcos.
  [...host.classList].forEach(cls=>{
    if(
      cls==='avatar-frame-shell' ||
      cls==='avatar-renderer-v39' ||
      cls==='equipped-avatar-frame' ||
      cls.startsWith('frame-rank-') ||
      cls.startsWith('frame-season-')
    ) host.classList.remove(cls);
  });
  host.classList.add('avatar-v42');

  // Reutilizamos el contenido de foto existente, pero quitamos su clase legacy.
  let photo=host.querySelector(':scope > .avatar-v42-photo');
  if(!photo){
    photo=host.querySelector(':scope > .avatar-photo-mask');
    if(photo){
      photo.classList.remove('avatar-photo-mask');
      photo.classList.add('avatar-v42-photo');
    }
  }
  if(!photo){
    photo=document.createElement('div');
    photo.className='avatar-v42-photo';
  ...
  // ÚNICO contenedor del marco. El PNG jamás se posiciona contra la página.
  let stage=host.querySelector(':scope > .avatar-v42-frame-stage');
  if(!stage){
    stage=document.createElement('div');
    stage.className='avatar-v42-frame-stage';
    const art=document.createElement('img');
    art.className='avatar-v42-frame-art';
    art.alt='';
  ...
  return {host,photo,stage,art:stage.querySelector(':scope > .avatar-v42-frame-art')};
}

function setAvatarPhotoV42(host,photoUrl,initials='TT',alt='Foto de perfil'){
  const parts=normalizeAvatarHostV42(host);
  if(!parts)return;
  let img=parts.photo.querySelector('img');
  let fallback=parts.photo.querySelector('span');
  if(!img){
    img=document.createElement('img');
    parts.photo.prepend(img);
  }
  if(!fallback){
    fallback=document.createElement('span');
    parts.photo.appendChild(fallback);
  }
  fallback.textContent=initials||'TT';

  if(photoUrl){
    img.src=photoUrl;
    img.alt=alt;
    parts.host.classList.add('has-photo');
    parts.host.dataset.hasPhoto='true';
  }else{
    img.removeAttribute('src');
    img.alt='Sin foto de perfil';
    parts.host.classList.remove('has-photo');
    parts.host.dataset.hasPhoto='false';
  }
}

function applyFrameGeometryV42(host,frameId,geometry=null){
  const parts=normalizeAvatarHostV42(host);
  if(!parts)return;

  const id=frameId||'none';
  const g=geometry||getFrameGeometryV42(id);
  ...
  // La etapa es SIEMPRE el mismo cuadrado que el AvatarHost.
  // Estas son las únicas tres variables de cada marco.
  parts.art.style.width=`${Number(g.frame_scale)}%`;
  parts.art.style.height=`${Number(g.frame_scale)}%`;
  parts.art.style.left=`${Number(g.frame_x)}%`;
  parts.art.style.top=`${Number(g.frame_y)}%`;
}

function renderAvatarV42(host,{frameId='none',photoUrl=null,initials='TT',alt='Foto de perfil',geometry=null}={}){
  if(!host)return;
  normalizeAvatarHostV42(host);
  setAvatarPhotoV42(host,photoUrl,initials,alt);
  applyFrameGeometryV42(host,frameId,geometry);
}

function ownAvatarFrameV42(){
  if(v35Flags?.is_test_admin && liveFramePreviewV42?.enabled){
    return liveFramePreviewV42.frameId;
  }
  return frameState?.equipped||'none';
}

function ownAvatarGeometryV42(){
  const id=ownAvatarFrameV42();
  if(v35Flags?.is_test_admin && liveFramePreviewV42?.enabled && liveFramePreviewV42.frameId===id){
    return liveFramePreviewV42;
  }
  return getFrameGeometryV42(id);
}

function renderOwnAvatarsV42(){
  if(!profile)return;
  const initials=((profile.first_name?.[0]||'')+(profile.last_name?.[0]||'')).toUpperCase()||'TT';
  const frameId=ownAvatarFrameV42();
  const geometry=ownAvatarGeometryV42();

  renderAvatarV42($('#homeAvatar'),{
    frameId,
    geometry,
...
function refreshOwnVisualsV42(){
  populate();
  renderOwnAvatarsV42();
  renderFrameGallery();
  if(v35Flags?.is_test_admin)setupAdminFrameLabV42();
}

function framePreviewMarkupV42(frameId){
  const initials=((profile?.first_name?.[0]||'')+(profile?.last_name?.[0]||'')).toUpperCase()||'TT';
  const g=getFrameGeometryV42(frameId);
  ...
  return `<div class="frame-preview-v42">
    <div class="frame-preview-photo-v42">${photo}</div>
    ${art?`<div class="frame-preview-stage-v42"><img src="${esc(art)}" alt="" style="width:${g.frame_scale}%;height:${g.frame_scale}%;left:${g.frame_x}%;top:${g.frame_y}%"></div>`:''}
  </div>`;
}

async function loadAvatarCalibrationsV42(){
  try{
    const rows=await getFrameCalibrationsV36();
    frameCalibrationsV42=new Map((rows||[]).map(x=>[x.frame_id,x]));
    renderOwnAvatarsV42();
    renderFrameGallery();
    if(v35Flags?.is_test_admin)setupAdminFrameLabV42();
  }catch(e){
    console.error('V42 calibraciones',e);
  }
}

function updateVisibleFrameGeometryV42(frameId,row){
  document.querySelectorAll(`.avatar-v42[data-frame-id="${CSS.escape(frameId)}"]`).forEach(host=>{
    // El admin conserva su draft local mientras lo está calibrando.
    if(
      v35Flags?.is_test_admin &&
      liveFramePreviewV42?.enabled &&
      liveFramePreviewV42.frameId===frameId &&
      (host.id==='homeAvatar'||host.id==='profileAvatar'||host.id==='adminAvatarPreviewV41')
    ) return;
    applyFrameGeometryV42(host,frameId,row);
  });
}

function startAvatarLiveSyncV42(){
  if(!session?.user?.id)return;
  if(stopAvatarLiveV42)stopAvatarLiveV42().catch?.(()=>{});

  stopAvatarLiveV42=subscribeVisualLiveV36(session.user.id,{
    onProfile:row=>{
      if(!row)return;
      profile={...profile,...row};
      refreshOwnVisualsV42();
    },
    onCosmetics:row=>{
      frameState={...frameState,equipped:row?.equipped_frame_id||null};
      if(!liveFramePreviewV42?.enabled)renderOwnAvatarsV42();
      renderFrameGallery();
    },
    onCalibration:row=>{
      if(!row?.frame_id)return;
      frameCalibrationsV42.set(row.frame_id,row);
      updateVisibleFrameGeometryV42(row.frame_id,row);
      renderFrameGallery();
      if(
        v35Flags?.is_test_admin &&
        $('#adminFrameSelectV41')?.value===row.frame_id &&
        !liveFramePreviewV42?.enabled
      ){
        setAdminFrameControlsV42(row);
      }
    }
  });
}

function publicFramedAvatarV42(p,frameId='none'){
  ...
  return `<div class="public-avatar-frame avatar-v42" data-frame-id="${esc(frameId)}">
    <div class="avatar-v42-photo">${photo}</div>
    ${art?`<div class="avatar-v42-frame-stage"><img class="avatar-v42-frame-art" src="${esc(art)}" alt="" aria-hidden="true" style="width:${g.frame_scale}%;height:${g.frame_scale}%;left:${g.frame_x}%;top:${g.frame_y}%"></div>`:''}
  </div>`;
}

function identityAvatarMarkupV42(p,frameId='none'){
  const initials=((p?.first_name?.[0]||'')+(p?.last_name?.[0]||'')).toUpperCase()||'TT';
  const g=getFrameGeometryV42(frameId);
  const art=frameId==='none'?'':frameArtUrlV37(frameId);
  const photo=p?.profile_photo_url
  ...
  return `<div class="identity-avatar avatar-v42" data-frame-id="${esc(frameId)}">
    <div class="avatar-v42-photo">${photo}</div>
    ${art?`<div class="avatar-v42-frame-stage"><img class="avatar-v42-frame-art" src="${esc(art)}" alt="" aria-hidden="true" style="width:${g.frame_scale}%;height:${g.frame_scale}%;left:${g.frame_x}%;top:${g.frame_y}%"></div>`:''}
  </div>`;
}
...
/* ============================================================
   V42 — LABORATORIO ADMIN
   El draft global sobrevive al cambio de pestaña.
   ============================================================ */

function clampV42(v,min,max){
  const n=Number(v);
...
function currentAdminFrameValuesV42(){
  return {
    frame_scale:clampV42($('#adminFrameScaleV41')?.value||100,10,180),
    frame_x:clampV42($('#adminFrameXV41')?.value||50,0,100),
    frame_y:clampV42($('#adminFrameYV41')?.value||50,0,100)
  };
}
function syncAdminControlPairV42(rangeId,numberId,labelId,sourceId){
  const range=$('#'+rangeId),number=$('#'+numberId),label=$('#'+labelId);
  if(!range||!number)return;
  const source=$('#'+sourceId);
  const value=clampV42(source?.value,Number(range.min),Number(range.max));
  range.value=value;
  number.value=value;
  if(label)label.textContent=`${fmtV42(value)}%`;
}
function setAdminFrameControlsV42(c){
  const rows=[
  ...
  rows.forEach(([rangeId,numberId,labelId,value])=>{
    const range=$('#'+rangeId),number=$('#'+numberId),label=$('#'+labelId);
...
function setLiveFramePreviewV42(frameId,geometry,enabled=true){
  liveFramePreviewV42={
    enabled:!!enabled,
    frameId,
    frame_scale:Number(geometry.frame_scale),
    frame_x:Number(geometry.frame_x),
    frame_y:Number(geometry.frame_y)
  };
  renderOwnAvatarsV42();
}

function clearLiveFramePreviewV42(){
  liveFramePreviewV42=null;
  renderOwnAvatarsV42();
}

function updateAdminFramePreviewV42(){
  ...
  const geometry=currentAdminFrameValuesV42();
  adminFrameDraftV42={frameId,...geometry};

  const initials=((profile?.first_name?.[0]||'')+(profile?.last_name?.[0]||'')).toUpperCase()||'TT';
  renderAvatarV42(preview,{
    frameId,
    geometry,
  ...
  if($('#adminLivePreviewV41')?.checked){
    setLiveFramePreviewV42(frameId,geometry,true);
  }
}

function setupAdminFrameLabV42(){
  ...
  const desired=
    (liveFramePreviewV42?.frameId && frames.some(f=>f.id===liveFramePreviewV42.frameId))
      ? liveFramePreviewV42.frameId
      : frames.some(f=>f.id===previous)
        ? previous
        : frames.some(f=>f.id===frameState.equipped)
          ? frameState.equipped
          : frames[0]?.id;

  if(desired)select.value=desired;

  if(!select.dataset.boundV42){
    select.dataset.boundV42='1';

    select.addEventListener('change',()=>{
      const g=getFrameGeometryV42(select.value);
      setAdminFrameControlsV42(g);
      updateAdminFramePreviewV42();
    });
    ...
    pairs.forEach(([rangeId,numberId,labelId])=>{
      ...
      range?.addEventListener('input',()=>{
        syncAdminControlPairV42(rangeId,numberId,labelId,rangeId);
        updateAdminFramePreviewV42();
      });
      number?.addEventListener('input',()=>{
        if(number.value==='')return;
        syncAdminControlPairV42(rangeId,numberId,labelId,numberId);
        updateAdminFramePreviewV42();
      });
  ...
  const initialGeometry=
    liveFramePreviewV42?.enabled && liveFramePreviewV42.frameId===select.value
      ? liveFramePreviewV42
      : getFrameGeometryV42(select.value);

  setAdminFrameControlsV42(initialGeometry);
  updateAdminFramePreviewV42();
}

async function saveAdminFrameV42(){
  ...
  try{
    const geometry=currentAdminFrameValuesV42();
    const old=frameCalibrationsV42.get(frameId)||{};

    const saved=await saveFrameCalibrationV36(frameId,{
      ...old,
      // Campos viejos: se dejan con valores neutros por compatibilidad DB.
      inner_diameter:AVATAR_BASE_DIAMETER_V42,
      photo_scale:100,
      photo_x:50,
      photo_y:50,
      plus_x:82,
    ...
    frameCalibrationsV42.set(frameId,saved);
    setLiveFramePreviewV42(frameId,geometry,$('#adminLivePreviewV41')?.checked!==false);
    updateVisibleFrameGeometryV42(frameId,saved);
    renderFrameGallery();
    setStatus(st,'Guardado: el PNG usa exactamente la misma posición y escala que ves acá.','ok');
  }catch(e){
    setStatus(st,e.message,'error');
  }
}

function resetAdminFrameV42(){
  setAdminFrameControlsV42({frame_scale:100,frame_x:50,frame_y:50});
  updateAdminFramePreviewV42();
  setStatus($('#adminFrameStatusV41'),'Vista restablecida a 100% / X 50 / Y 50. Guardá para hacerla oficial.','ok');
}
...
function populate(){
  ...
  // V42: única llamada para Inicio + Perfil.
  // Si existe un draft admin, populate() NO lo pisa al cambiar de pestaña.
  renderOwnAvatarsV42();
...
async function refreshCore(){
  ...
  await loadV35Flags();
  await loadAvatarCalibrationsV42();
  populate();
...
function activateTab(tab){
  ...
  if(tab==='admin')setupAdminFrameLabV42();
...
function renderIdentityShowcase(){
  ...
  box.innerHTML=`
    ${identityAvatarMarkupV42(profile,equippedFrame||'none')}
    ...
    box.innerHTML=`
      <section class="public-profile-v17">
        ...
        <div class="public-profile-v17-head">
          ${publicFramedAvatarV42(p,(cosmetics||publicPrefs)?.equipped_frame_id||'none')}
...
if($('#adminSaveFrameV41'))$('#adminSaveFrameV41').onclick=saveAdminFrameV42;
if($('#adminResetFrameV41'))$('#adminResetFrameV41').onclick=resetAdminFrameV42;

if($('#adminRestoreEquippedV41'))$('#adminRestoreEquippedV41').onclick=()=>{
  clearLiveFramePreviewV42();
  if($('#adminLivePreviewV41'))$('#adminLivePreviewV41').checked=false;
  setStatus($('#adminFrameStatusV41'),'Mostrando el marco realmente equipado.','ok');
};

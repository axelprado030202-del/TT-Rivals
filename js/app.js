...
function finishBootV572(){
  document.body.classList.remove('booting-v572');
  const splash=$('#appBootSplashV572');
  if(!splash)return;
  splash.classList.add('is-finished');
  setTimeout(()=>splash.remove(),220);
}

function setBootTextV572(text){
  const el=$('#appBootTextV572');
...
async function loadApp(uid,p=null){
  // V57.2 — ARRANQUE PROGRESIVO
  // Sólo perfil + ratings bloquean la primera pintura.
  // El resto se hidrata después, en paralelo, sin retener al usuario.
  profile=p||await getMyProfile(uid);
  ratings=await getMyRatings(uid);

  // Ya tenemos suficiente información para mostrar Inicio.
  populate();
  showMain();
  finishBootV572();

  // Todo lo secundario se resuelve en paralelo.
  // Esto reemplaza seis esperas consecutivas que antes atrasaban la entrada.
  Promise.all([
    getRankTiers().then(rows=>{rankTiers=rows||[]}).catch(e=>{console.error(e);rankTiers=[]}),
    loadSocialState(),
    loadExperienceSettings(),
    loadV35Flags(),
    loadFrameFitsV44()
  ]).then(async()=>{
    // Repintado ya con social, preferencias, Admin, marcos, etc.
    populate();
    renderOwnAvatarsV44();
    ...
    // Dashboard y páginas secundarias también cargan en paralelo.
    await Promise.all([
      loadRanking(),
      loadChallenges(),
      loadMatches(),
    ...
    // La app ya está utilizable aunque una carga secundaria falle.
    console.error('Carga secundaria V57.2:',err);
  });
}
async function route(prefetchedSession=null){
  ...
  setBootTextV572('Preparando tu perfil…');
...
(async()=>{
  ...
  try{
    ...
    // Resolver la sesión antes de mostrar Welcome.
    setBootTextV572('Comprobando tu sesión…');
    ...
    if(recoveryHint){
      ...
      // Menor espera: sólo para permitir que supabase-js procese el enlace.
    ...
    console.debug(
      `TT Rivals V57.2 · primera vista en ${Math.round(performance.now()-bootStarted)} ms`
    );
  }catch(e){
    console.error(e);
    showView('welcomeView');
    setStatus($('#globalStatus'),'Hubo un problema al cargar la app.','error');
    finishBootV572();
  }
})();

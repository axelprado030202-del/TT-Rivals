...
import {createTeamTournamentV32,getTeamTournamentV32,listMyTeamTournamentsV32,submitTeamTournamentMatchResultV32,createTeamTiebreakV32,finalizeTeamTournamentDrawV32} from './team_tournaments.js';
...
function renderV28Dashboard(){
  ...
  if($('#v28EventBadge'))$('#v28EventBadge').textContent=ev.badge;
...
async function searchTeamPlayerV32(input){
  ...
  teamSearchTimerV32=setTimeout(async()=>{
    try{
      ...
      const used=new Set([...teamDraftV32.home,...teamDraftV32.away].filter(Boolean).map(p=>p.id));
      users=users.filter(u=>!used.has(u.id)).slice(0,8);
...
function openTeamTournamentBuilderV32(){
  ...
  $('#teamTournamentDetailV32').classList.add('hidden');
...
async function openTeamTournamentDetailV32(id){
  try{
    ...
    $('#tournamentModeChooser').classList.add('hidden');
    $('#teamTournamentBuilderV32').classList.add('hidden');
    $('#tournamentPresetChooser').classList.add('hidden');
    $('#tournamentBuilder').classList.add('hidden');
    $('#tournamentDetail').classList.add('hidden');
...
function renderTeamTournamentDetailV32(){
  ...
  const statusLabel=t.status==='completed'?'FINALIZADO':t.status==='draw'?'EMPATE':t.status==='tied_pending'?'5–5 · DECISIÓN':'EN JUEGO';
  $('#teamTournamentScoreboardV32').innerHTML=`
    <div class="team-score-status-v32">${statusLabel}</div>
    ...
    <p>Primero en llegar a <strong>6 partidos</strong> · Todos los encuentros al mejor de 3 sets.</p>`;

  $('#teamTournamentMatchesV32').innerHTML=(data.matches||[]).map(m=>{
    const home=teamParticipantLabelV32(m,map,'home'),away=teamParticipantLabelV32(m,map,'away');
    ...
    return `<article class="team-match-card-v32 ${m.status} ${m.winner_side?`winner-${m.winner_side}`:''}">
      ...
      <div class="team-match-side-v32 ${m.winner_side==='home'?'winner':''}"><strong>${esc(home)}</strong>${m.status==='completed'?`<b>${m.home_sets}</b>`:''}</div>
      <span class="team-match-vs-v32">VS</span>
      <div class="team-match-side-v32 away ${m.winner_side==='away'?'winner':''}"><strong>${esc(away)}</strong>${m.status==='completed'?`<b>${m.away_sets}</b>`:''}</div>
      <div class="team-match-action-v32">
        ${m.status==='pending'&&t.is_host?`<button type="button" data-team-match-result="${m.id}">Cargar resultado</button>`:m.status==='completed'?'<span>✓ Finalizado</span>':'<span>No jugado</span>'}
      </div>
    </article>`;
  }).join('');
  ...
  if(t.status==='tied_pending'&&t.is_host){
    ...
    tb.innerHTML=`<div class="team-tiebreak-decision-v32"><div><p class="muted-label">5–5</p><h3>¿Cómo termina la serie?</h3><span>Podés jugar un dobles decisivo o cerrar oficialmente el torneo como empate.</span></div><div><button type="button" data-open-team-tiebreak>⚔ DESEMPATE</button><button class="draw" type="button" data-finalize-team-draw>FINALIZAR 5–5</button></div></div>`;
  ...
  if(['completed','draw'].includes(t.status)){
    ...
    summary.innerHTML=`<div class="section-title-row"><div><p class="muted-label">RESUMEN FINAL</p><h3>${t.status==='draw'?'Serie empatada':`🏆 ${esc(winner)}`}</h3></div><strong>${t.home_score}–${t.away_score}</strong></div>
      <div class="team-elo-summary-v32">${(data.elo_summary||[]).map(s=>`<article><strong>${esc(s.name)}</strong><small>${s.matches_played} PJ · ${s.wins}V–${s.losses}D</small><div><span>Individual <b class="${s.individual_change>=0?'positive':'negative'}">${s.individual_change>=0?'+':''}${s.individual_change}</b></span><span>Dobles <b class="${s.doubles_change>=0?'positive':'negative'}">${s.doubles_change>=0?'+':''}${s.doubles_change}</b></span></div><em>TOTAL ${s.total_change>=0?'+':''}${s.total_change} Elo</em></article>`).join('')}</div>`;
...
document.addEventListener('click',async e=>{
  ...
  if(e.target.closest('[data-finalize-team-draw]')){
    if(confirm('¿Finalizar oficialmente la serie como empate 5–5?')){
      try{await finalizeTeamTournamentDrawV32(selectedTeamTournamentV32.tournament.id);selectedTeamTournamentV32=await getTeamTournamentV32(selectedTeamTournamentV32.tournament.id);renderTeamTournamentDetailV32()}catch(err){alert(err.message)}
    }
    return;
  }
...
$('#openTeamTournamentV32').onclick=openTeamTournamentBuilderV32;
$('#backFromTeamTournamentV32').onclick=()=>{resetTournamentWizard();loadTournamentHubV30()};
$('#backFromTeamDetailV32').onclick=()=>{selectedTeamTournamentV32=null;openTeamTournamentBuilderV32()};
$('#refreshTeamTournamentsV32').onclick=loadTeamTournamentListV32;
$('#closeTeamMatchModalV32').onclick=()=>{$('#teamMatchModalV32').classList.add('hidden');currentTeamMatchV32=null;syncModalScrollLock()};
$('#teamMatchModalV32').addEventListener('click',e=>{if(e.target===$('#teamMatchModalV32'))$('#closeTeamMatchModalV32').click()});
$('#closeTeamTiebreakModalV32').onclick=()=>{$('#teamTiebreakModalV32').classList.add('hidden');syncModalScrollLock()};
...
$('#teamTournamentFormV32').onsubmit=async e=>{
  ...
  if(new Set(all.map(x=>x.id)).size!==8)return setStatus(st,'Los 8 lugares deben ser usuarios diferentes.','error');
  ...
  if(Object.values(pairIds).some(p=>!p[0]||!p[1]||p[0]===p[1]))return setStatus(st,'Completá las cuatro parejas de dobles sin repetir jugador dentro de una pareja.','error');
...
$('#teamTiebreakFormV32').onsubmit=async e=>{
  ...
  if(home[0]===home[1]||away[0]===away[1])return setStatus($('#teamTiebreakStatusV32'),'Elegí dos jugadores diferentes por equipo.','error');
  try{

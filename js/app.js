...
let v60State={challenges:[],matches:[],recommended:[],activityItems:[],activityFilter:'attention',historyModality:'all',historySeason:'all',historyDateFrom:'',historyDateTo:'',historySort:'recent',recentPresence:new Map(),historySeasons:[],adminCategory:'disputes',lastDiagnostics:null};
...
function pushRewardV60(reward,{toast=true}={}){
  ...
  if(toast)showRewardToastV60(row);
  return row;
}
...
async function loadV35Flags(){
  ...
  $('#checkAppUpdateV59')?.classList.toggle('hidden',!v35Flags.is_test_admin);
...
async function loadRanking(){
  ...
  try{
    ...
    list.innerHTML=regularRows.length?regularRows.map(x=>{
      ...
      return `<article class="rank-row rank-row-button v60-rank-row ${x.profile.id===session.user.id?'is-me':''}">
        <button class="v60-rank-profile" data-open-player="${p.id}" type="button">
        ...
        ${p.id===session.user.id?'':`<button class="v60-rank-challenge" data-quick-challenge="${p.id}" data-name="${esc(p.first_name)} ${esc(p.last_name)}" data-user="${esc(p.username)}" type="button" aria-label="Desafiar a ${esc(p.first_name)}">⚔</button>`}
      </article>`;
...
function renderHomePriorityV60(){
  ...
  if(confirmation){
    ...
    html=`<div class="v60-priority-kicker"><span>TODO AL DÍA</span><b>Elegí tu próxima acción</b></div><div class="v60-priority-main"><span class="v60-priority-icon">✦</span><div><strong>No tenés acciones pendientes</strong><small>Explorá el ranking o buscá un nuevo rival.</small></div></div><div class="v60-priority-actions"><button class="primary" data-go-tab="play" type="button">Buscar rival</button><button data-go-tab="ranking" type="button">Ver ranking</button></div>`;
  }
  box.innerHTML=html;
}
...
function renderProtectionV58(){
  ...
  if($('#homeProtectionBarV58'))$('#homeProtectionBarV58').style.width=`${pct}%`;
  ...
  if($('#profileProtectionBarV58'))$('#profileProtectionBarV58').style.width=`${pct}%`;
...
async function renderDisputeModalV58(){
  ...
  if(d.status==='admin_requested'){
    actions.innerHTML='<div class="compact-empty">La disputa fue enviada al Administrador. Recibirán una notificación cuando quede resuelta.</div>';
    return;
  }
...
async function loadAdminDisputesV58(){
  ...
  try{
    ...
    box.innerHTML=rows.length?rows.map(d=>`<button type="button" class="admin-dispute-row-v58" data-admin-open-dispute-v58="${d.match_id}"><span>⚖</span><div><strong>${esc(d.player1_name||'Jugador')} vs ${esc(d.player2_name||'Jugador')}</strong><small>${esc(disputeStatusLabelV58(d.status))} · ${disputeFormatLabelV58(d.match_format)}</small></div><b>Resolver →</b></button>`).join(''):'<div class="compact-empty">No hay disputas abiertas que requieran atención.</div>';
  }catch(err){box.innerHTML=`<div class="compact-empty">${esc(err.message)}</div>`}
}
  ...
  try{
    ...
    box.innerHTML=rows.length?rows.map(x=>{
      ...
      return `<article class="v59-integrity-row risk-${Number(x.risk_score||0)>=75?'high':Number(x.risk_score||0)>=50?'medium':'low'}">
        ...
        <b class="v59-integrity-id">#${x.match_id}</b>
      </article>`;
...
function setAdminCategoryV60(category='disputes'){
  const allowed=new Set(['disputes','integrity','multi','users','suspicious','system']);
...
function matchCard(m){
  ...
  return`<div class="match-row v59-match-row"><div class="match-meta"><strong>vs ${esc(other?.first_name)} ${esc(other?.last_name)}</strong><small>${fmt} · ${mode}${result}${finish}</small><div class="v59-match-state-line"><span class="challenge-status ${cls}">${state}</span>${timing}</div></div>${actions}</div>`
}
...
function renderEquippedTitle(){
  ...
  if(hero){hero.classList.remove('hidden');hero.innerHTML=`<span>${esc(t.icon||'✦')}</span><strong>${esc(t.name)}</strong>`}
}
async function openTitleSelector(){
  ...
  try{
    ...
    trackTitleUnlocksV60(titleState.items||[]);
    const items=[{id:'',icon:'○',name:'Sin título',description:'No mostrar ningún título bajo tu nombre.',rarity:'common',unlocked:true,progress:100,progress_label:'Disponible',source:'system'},...(titleState.items||[])];
    box.innerHTML=items.map(t=>{
      ...
      return `<article class="title-choice title-choice-v58 ${t.unlocked?'unlocked':'locked'} ${active?'active':''}">
        ...
        <div class="title-choice-copy-v58">
          ...
          ${t.source==='v58'?`<div class="title-progress-v58"><div><span>${esc(t.progress_label||'Progreso')}</span><b>${Math.round(progress)}%</b></div><div class="title-progress-track-v58"><span style="width:${progress}%"></span></div></div>`:''}
        </div>
...
async function saveCurrentReview(){
  ...
  try{
    await submitPlayerReviewV58(Number(reviewTargetMatch.id),selectedReviewStars,[...selectedReviewTagsV58]);
      ...
      <section class="public-profile-v17">
        ...
        <div class="public-profile-v17-head">
          ...
          <div class="public-profile-v17-identity">
            ...
            ${(()=>{const t=(publicTitles.items||[]).find(x=>x.id===publicTitles.equipped);return t?`<div class="public-equipped-title-v58 rarity-${t.rarity}"><span>${esc(t.icon||'✦')}</span><strong>${esc(t.name)}</strong></div>`:''})()}
            <p>@${esc(p.username)}</p>
...
document.addEventListener('click',async e=>{
  ...
  const resp=e.target.closest('[data-response]');if(resp){
    ...
    await withActionLockV60(`challenge-response:${id}:${response}`,resp,async()=>{try{await respondToChallenge(id,response);await Promise.all([loadChallenges(),loadMatches(),loadLiveNotifications(),loadActivityCenter()]);recoverPageScrollIfIdle()}catch(err){alert(err.message)}},{loadingText:response==='accepted'?'Aceptando…':'Procesando…'});
    return
  }
  ...
  const confirm=e.target.closest('[data-confirm-match]');if(confirm){
    ...
    }catch(err){alert(err.message)}},{loadingText:'Confirmando…'});
    return
  }
  const dispute=e.target.closest('[data-dispute-match]');if(dispute){
    ...
    }catch(err){alert(err.message)}},{loadingText:'Abriendo disputa…'});
    return
  }
});
...
$('#matchResultForm').onsubmit=async e=>{
  ...
  },{loadingText:'Guardando resultado…'});
};

...
async function refreshCore(){ratings=await getMyRatings(session.user.id);try{rankTiers=await getRankTiers()}catch(e){}populate();await Promise.all([loadRanking(),loadChallenges(),loadMatches(),loadHistory()])}
async function loadApp(uid,p=null){profile=p||await getMyProfile(uid);ratings=await getMyRatings(uid);try{rankTiers=await getRankTiers()}catch(e){console.error(e);rankTiers=[]}populate();showMain();await Promise.all([loadRanking(),loadChallenges(),loadMatches(),loadHistory()])}
...
async function loadChallenges(){
  ...
  $('#notificationBadge').textContent=rec.length;
}
...
function matchCard(m){
  ...
  return`<div class="match-row"><div class="match-meta"><strong>vs ${esc(other?.first_name)} ${esc(other?.last_name)}</strong><small>${fmt}${result}${finish}</small><span class="challenge-status ${cls}">${state}</span></div>${actions}</div>`
}
      ...
      return `<article class="history-match-card ${type}">
        ...
        <div class="history-match-main">
          ...
          <div class="history-opponent">
            ...
            <span>@${esc(other?.username||'usuario')} · ${fmt} · ${formatMatchDate(m)}</span>
          </div>
...
async function openPublicPlayerProfile(userId){
  ...
  try{
    const [p,prs,reviews]=await Promise.all([
      getPlayerProfile(userId),
      getPlayerRatings(userId),
      getReviewsForUser(userId)
    ]);
    const ind=prs.find(r=>r.modality==='individual')||{rating:1000,matches_played:0,wins:0,losses:0};
    const dob=prs.find(r=>r.modality==='dobles')||{rating:1000};
    const rep=averageReviews(reviews);
    const rank=rankForRating(ind.rating);

    const h2h=(socialState.matches||[]).filter(m=>
      m.result_status==='confirmed' &&
      (m.player1_id===userId||m.player2_id===userId)
    );
    const myWins=h2h.filter(m=>m.winner_id===session.user.id).length;
    const theirWins=h2h.filter(m=>m.winner_id===userId).length;

    box.innerHTML=`
      <div class="public-profile-head">
        ${avatarHtml(p,'public-profile-avatar')}
        <div>
          <p class="muted-label">JUGADOR</p>
          <h2>${esc(p.first_name)} ${esc(p.last_name)}</h2>
          <p>@${esc(p.username)}</p>
        </div>
        <span class="public-rank-chip ${rankCss(rank)}">${rank}</span>
      </div>

      <div class="public-profile-metrics">
        <article><span>Individual</span><strong>${ind.rating}</strong><small>Elo</small></article>
        <article><span>Dobles</span><strong>${dob.rating}</strong><small>Elo</small></article>
        <article><span>Reputación</span><strong>${rep.count?`★ ${rep.average.toFixed(1)}`:'—'}</strong><small>${rep.count} valoraciones</small></article>
        <article><span>Win rate</span><strong>${ind.matches_played?`${((ind.wins/ind.matches_played)*100).toFixed(0)}%`:'—'}</strong><small>${ind.matches_played} partidos</small></article>
      </div>

      ${userId!==session.user.id?`
        <section class="public-h2h">
          <p class="muted-label">CARA A CARA</p>
          <div><strong>Vos ${myWins}</strong><span>${h2h.length} enfrentamientos</span><strong>${theirWins} ${esc(p.first_name)}</strong></div>
        </section>
        <button class="btn btn-start" type="button" data-public-challenge="${p.id}" data-name="${esc(p.first_name)} ${esc(p.last_name)}" data-user="${esc(p.username)}">DESAFIAR A ${esc(p.first_name).toUpperCase()}</button>
      `:''}
    `;
  }catch(err){
    console.error(err);

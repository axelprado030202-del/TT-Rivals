...
import {getFrameCalibrationsV36,saveFrameCalibrationV36,resetFrameCalibrationV36,subscribeVisualLiveV36,getPublicAdminFlagV37} from './v36_live.js';
...
const defaultFrameCalibrationV36={
  photo_scale:50,photo_x:50,photo_y:50,frame_scale:100,frame_x:50,frame_y:50,plus_x:82,plus_y:68
};
...
function calibrationStyleV36(frameId,override=null){
  ...
  return [
    `--avatar-photo-size:${Number(c.photo_scale)}%`,
...
function applyCalibrationToElementV36(el,frameId,override=null){
  if(!el)return;
  const c=override||getFrameCalibrationV36(frameId);
  el.style.setProperty('--avatar-photo-size',`${Number(c.photo_scale)}%`);
...
function framePreviewMarkupV36(frameId){
  ...
  return `<div class="frame-preview-v37" data-frame-calibration-id="${esc(frameId)}">
    <div class="frame-gallery-photo-v37" style="width:${Number(c.photo_scale)}%;height:${Number(c.photo_scale)}%;left:${Number(c.photo_x)}%;top:${Number(c.photo_y)}%">${has?`<img src="${esc(profile.profile_photo_url)}" alt="">`:`<span>${esc(initials)}</span>`}</div>
...
function readFrameDraftV36(){
  return {
    photo_scale:Number($('#calPhotoScaleV36')?.value||50),
...
function setFrameDraftInputsV36(c){
  const vals={
    calPhotoScaleV36:c.photo_scale,calPhotoXV36:c.photo_x,calPhotoYV36:c.photo_y,
...
function updateFrameCalibratorPreviewV36(){
  ...
  if(photo){
    photo.style.width=`${draft.photo_scale}%`;
    photo.style.height=`${draft.photo_scale}%`;
...
function setupFrameCalibratorV36(){
  ...
  if(!select.dataset.boundV36){
    ...
    ['calPhotoScaleV36','calPhotoXV36','calPhotoYV36','calFrameScaleV36','calFrameXV37','calFrameYV37','calPlusXV36','calPlusYV36'].forEach(id=>{
...
async function loadApp(uid,p=null){
  ...
  populate();
  showMain();
  presenceManagerV35=createPresenceManagerV35(session.user.id,ids=>{onlineUserIdsV35=new Set([...ids].map(String));decorateOnlineIndicatorsV35()});

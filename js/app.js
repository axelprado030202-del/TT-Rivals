...
function framePreviewMarkupV44(frameId){
  const initials=((profile?.first_name?.[0]||'')+(profile?.last_name?.[0]||'')).toUpperCase()||'TT';
  ...
  if(art&&fit){
    ...
    const widthPct=(fit.png_width/fit.hole_diameter)*100*s;
    const heightPct=(fit.png_height/fit.hole_diameter)*100*s;
    const ax=(fit.hole_center_x/fit.png_width)*100;
    const ay=(fit.hole_center_y/fit.png_height)*100;
    frame=`<div class="tt-frame-preview-stage-v44"><img src="${esc(art)}" alt="" style="width:${widthPct}%;height:${heightPct}%;left:${50+fit.fine_x}%;top:${50+fit.fine_y}%;transform:translate(-${ax}%,-${ay}%)"></div>`;
  }

  return `<div class="tt-frame-preview-v44">
    <div class="tt-frame-preview-photo-v44">${photo}</div>
    ${frame}
  </div>`;
}

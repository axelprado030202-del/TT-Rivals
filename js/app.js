...
import {getMyProfile,getMyRatings,completeSportsProfile,getClubsV47,ensureClubV47,getClubsV49,getClubsV50,suggestClubsV49,suggestClubsV50,ensureClubV49,setMyClubV49,getMyClubV49,adminListClubsV49,adminMergeClubsV49,adminRenameClubV49,getRanking,searchPlayers,getRatingHistory,getRankTiers,setProfilePhotoUrl,uploadProfilePhoto,deleteProfilePhotoByUrl} from './profile.js';
...
function renderFrameGallery(){
  ...
  box.innerHTML=rows.map(f=>{
    ...
    const cardTier=f.id==='season_first'?'season-card-first':f.id==='season_second'?'season-card-second':f.id==='season_third'?'season-card-third':f.id==='season_elite'?'season-card-elite':'';
...
async function route(){
  ...
  if(!session?.user){showView('welcomeView');setStatus($('#globalStatus'),'');return}
...
function activateTab(tab){
  ...
  if(tab==='admin'){
    setupAdminFrameLabV44();
    if(v35Flags?.is_test_admin)loadAdminClubsV49().catch(err=>console.warn('Admin clubes V49:',err));
  }
...
async function loadAvailableClubsV49(preferred=''){
  ...
  try{
    availableClubsV49=await getClubsV50();
    const options=[
...
async function renderClubSuggestionsV49(input,box,context){
  ...
  try{
    const rows=await suggestClubsV50(q,8);
    ...
    box.innerHTML=`<div class="club-suggestions-head-v49"><strong>¿Ya existe?</strong><small>Elegí el club oficial para evitar duplicados.</small></div>`+
      rows.map(r=>`<button type="button" data-use-club-v49="${r.id}" data-club-context-v49="${context}">
        <span>🏓</span>
        <div><strong>${esc(r.name)}</strong><small>${r.department?`${esc(r.department)}${r.locality&&r.locality!==r.department?` · ${esc(r.locality)}`:''} · `:''}${r.reason==='sigla'?'Coincide por sigla':r.reason==='alias'?'Nombre alternativo reconocido':'Nombre parecido'}</small></div>
        <b>USAR</b>
      </button>`).join('');
    box.classList.remove('hidden');
  }catch(err){
...
async function loadSettingsClubV49(){
  ...
  const [clubs,current]=await Promise.all([
    getClubsV50(),
    getMyClubV49().catch(()=>({club_id:null,name:profile?.club_name||'N/A'}))
  ]);
  ...
  if(hint)hint.textContent=myClubV49.club_id
    ?`Vinculado al club oficial: ${myClubV49.name}.`
...
async function resolveSelectedClubV49(selectId,customInputId){
  ...
  if(value==='Otro'){
    ...
    const result=await ensureClubV49(raw);
...
$('#sportsProfileForm').onsubmit=async e=>{
  ...
  try{
    ...
    await setMyClubV49(clubResult.club_id??null);
...
$('#competitiveProfileSettingsForm').onsubmit=async e=>{
  ...
  try{
    ...
    await setMyClubV49(clubResult.club_id??null);
    profile=await getMyProfile(session.user.id);
    myClubV49={club_id:clubResult.club_id??null,name:clubResult.name};
...
function renderAdminClubListV49(){
  ...
  const q=normalizeClubNameClientV49($('#adminClubSearchV49')?.value||'');
  const targetId=String(target.value||'');

  const rows=(adminClubsV49||[]).filter(c=>{
    if(!q)return true;
    const hay=normalizeClubNameClientV49(`${c.name} ${(c.aliases||[]).join(' ')}`);
    return hay.toLowerCase().includes(q.toLowerCase());
  });

  box.innerHTML=rows.length?rows.map(c=>{
    const isTarget=String(c.id)===targetId;
    const aliases=(c.aliases||[]).filter(a=>normalizeClubNameClientV49(a)!==normalizeClubNameClientV49(c.name));
    return `<label class="admin-club-row-v49 ${isTarget?'is-target':''}">
      ...
      <div>
        ...
        <small>${Number(c.member_count||0)} jugador${Number(c.member_count||0)===1?'':'es'}${aliases.length?` · Alias: ${esc(aliases.slice(0,3).join(', '))}`:''}</small>
      </div>
      <b>#${c.id}</b>
    </label>`;
...
async function loadAdminClubsV49(){
  ...
  adminClubsV49=await adminListClubsV49();

  const previous=target.value;
  target.innerHTML=adminClubsV49.map(c=>`<option value="${c.id}">${esc(c.name)} · ${Number(c.member_count||0)} jugadores</option>`).join('');
...
$('#adminRenameClubButtonV49')?.addEventListener('click',async()=>{
  ...
  try{
    setStatus(st,'Actualizando nombre oficial…');
    await adminRenameClubV49(id,name);
    await Promise.all([loadAdminClubsV49(),loadAvailableClubsV49().catch(()=>{})]);
    profile=await getMyProfile(session.user.id);
    populate();
    setStatus(st,'Nombre oficial actualizado. El nombre anterior quedó guardado como alias.','ok');
...
$('#adminMergeClubsButtonV49')?.addEventListener('click',async()=>{
  const st=$('#adminClubStatusV49');
  const targetId=$('#adminClubTargetV49')?.value;
  const sourceIds=[...document.querySelectorAll('[data-admin-club-source-v49]:checked')].map(x=>Number(x.dataset.adminClubSourceV49));
  ...
  const sourceNames=(adminClubsV49||[]).filter(c=>sourceIds.includes(Number(c.id))).map(c=>c.name);
  const ok=confirm(`Se conservará "${target?.name||'el club oficial'}" y se fusionarán: ${sourceNames.join(', ')}. Los usuarios no perderán datos. ¿Continuar?`);
  if(!ok)return;

  const aliases=($('#adminClubAliasesV49')?.value||'').split(',').map(x=>normalizeClubNameClientV49(x)).filter(Boolean);

  try{
    setStatus(st,'Fusionando clubes…');
    const result=await adminMergeClubsV49(targetId,sourceIds,aliases);
    ...
    profile=await getMyProfile(session.user.id);
    populate();
    setStatus(st,`Listo. ${result?.merged_clubs||sourceIds.length} club${(result?.merged_clubs||sourceIds.length)===1?'':'es'} fusionado${(result?.merged_clubs||sourceIds.length)===1?'':'s'} en ${result?.target_name||target?.name}.`,'ok');
  }catch(err){setStatus(st,err.message,'error')}
});

...
import {getReviewsForUser,getReviewsAuthoredByUser,submitPlayerReview,getPlayerProfile,getPlayerRatings,followPlayer,unfollowPlayer,getFollowingIds,getFollowingRanking,getPublicPlayerCard,getFollowingFeed,setPrimaryRival,clearPrimaryRival,getMyPrimaryRival} from './social.js';
...
function buildAchievements({matches=[],streak={current:0,max:0},history=[],rating=1000}){
  ...
  const maxRating=maxHistoricalRating(history,rating);
  return [
    {id:'first_match',icon:'◉',name:'Primer saque',desc:'Jugá tu primer partido',unlocked:confirmed.length>=1},
    {id:'first_win',icon:'✓',name:'Primera victoria',desc:'Conseguí tu primera victoria',unlocked:wins>=1},
    {id:'streak3',icon:'🔥',name:'En racha',desc:'Ganá 3 partidos seguidos',unlocked:streak.max>=3},
    {id:'streak5',icon:'⚡',name:'Imparable',desc:'Ganá 5 partidos seguidos',unlocked:streak.max>=5},
    {id:'streak10',icon:'♛',name:'Dominio total',desc:'Ganá 10 partidos seguidos',unlocked:streak.max>=10},
    {id:'matches10',icon:'10',name:'Competidor',desc:'Jugá 10 partidos',unlocked:confirmed.length>=10},
    {id:'matches50',icon:'50',name:'Veterano',desc:'Jugá 50 partidos',unlocked:confirmed.length>=50},
    {id:'matches100',icon:'100',name:'Centenario',desc:'Jugá 100 partidos',unlocked:confirmed.length>=100},
    {id:'silver',icon:'◇',name:'Rango Plata',desc:'Alcanzá 1100 Elo',unlocked:maxRating>=1100},
    {id:'gold',icon:'◆',name:'Rango Oro',desc:'Alcanzá 1250 Elo',unlocked:maxRating>=1250},
    {id:'platinum',icon:'✦',name:'Rango Platino',desc:'Alcanzá 1400 Elo',unlocked:maxRating>=1400},
    {id:'diamond',icon:'✧',name:'Rango Diamante',desc:'Alcanzá 1600 Elo',unlocked:maxRating>=1600},
  ];
}
...
async function loadSocialState(){
  ...
  const [matches,history,reviewsReceived,reviewsAuthored,follows,rivalId]=await Promise.all([
  ...
  socialState.achievements=buildAchievements({
    matches,streak:socialState.streak,history,rating:getRating('individual').rating
  });
...
async function loadRanking(){
  ...
  try{
    ...
    list.innerHTML=rows.length?rows.map(x=>{
      ...
      return `<button class="rank-row rank-row-button" data-open-player="${p.id}" type="button">
        <div class="rank-pos">${m}</div>
        ${avatarHtml(p,'ranking-avatar')}
        <div class="rank-name"><strong>${esc(p.first_name)} ${esc(p.last_name)}</strong><small>@${esc(p.username)}</small></div>
        <div class="rank-rating">${x.rating}<small>Elo</small></div>
      </button>`;
...
async function openPublicPlayerProfile(userId){
  ...
  try{
    const p=await getPublicPlayerCard(userId);
    ...
    box.innerHTML=`
      <section class="public-profile-v17">
        ...
        <div class="public-profile-v17-head">
          ...
          ${!isMe?`<button id="followPlayerButton" class="follow-player-button ${p.is_following?'following':''}" data-follow-player="${p.id}" data-is-following="${p.is_following?'1':'0'}" type="button">${p.is_following?'Siguiendo ✓':'＋ Seguir'}</button>`:''}
        </div>
        ...
        <div class="public-profile-highlight-grid">
          ...
          <article><span>Logros</span><strong>${p.achievement_count}</strong><small>de ${p.achievement_total}</small></article>
        </div>
...
$('#closePlayerProfileModal').onclick=()=>$('#playerProfileModal').classList.add('hidden');

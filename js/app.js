...
import {getMyProfile,getMyRatings,completeSportsProfile,getRanking,searchPlayers,getRatingHistory,getRankTiers} from './profile.js';
...
function populate(){
  ...
  $('#homeUsername').textContent=`@${profile.username}`; $('#homeCategory').textContent=rankForRating(ind.rating);
  $('#homeIndividualRank').textContent=rankForRating(ind.rating);$('#homeDoublesRank').textContent=rankForRating(dob.rating);
  ...
  $('#profileIndividualRank').textContent=rankForRating(ind.rating);$('#profileDoublesRank').textContent=rankForRating(dob.rating);
...
function openMatchModal(matchId,matchesCache=[]){
  ...
  $('#setInputs').innerHTML=Array.from({length:fmt},(_,i)=>`<div class="set-row result-set-row"><strong>Set ${i+1}</strong><label>${esc(m.player1?.first_name||'J1')}<input type="number" min="0" max="99" data-p1-set="${i+1}"></label><label>${esc(m.player2?.first_name||'J2')}<input type="number" min="0" max="99" data-p2-set="${i+1}"></label><label class="unplayed-toggle"><input type="checkbox" data-unplayed-set="${i+1}"><span>Set no jugado</span></label></div>`).join('');
  $('#setInputs').insertAdjacentHTML('beforeend','<div class="abandonment-hint">⚠️ Si el último set tiene puntos pero ninguno llegó a 11, TT Rivals lo interpreta como <strong>abandono</strong>. El jugador con menos puntos será marcado como quien abandonó.</div>');
...
function rankIcon(name){
  return ({Bronce:'🥉',Plata:'🥈',Oro:'🥇',Platino:'💠',Diamante:'💎'})[name]||'🏅';
}
async function openRankTable(){
  ...
  $('#rankTableCurrent').innerHTML=`
    <strong>${rankIcon(current)} Rango actual: ${esc(current)}</strong>
  ...
  $('#rankTierList').innerHTML=rankTiers.map((t,i)=>{
    ...
    return `<div class="rank-tier-row ${t.name===current?'current':''}">
      <div class="rank-tier-icon">${rankIcon(t.name)}</div>
      <div><strong>${esc(t.name)}</strong><small>${range} puntos de ranking</small></div>
      <div class="rank-threshold">${t.min_rating}</div>
    </div>`;
  }).join('');

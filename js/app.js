...
let frameCalibrationsV43=new Map(),liveFramePreviewV43=null,stopAvatarLiveV43=null;
let adminUserIdsV38=new Set(),universalPlayerObserverV38=null;
const AVATAR_BASE_DIAMETER_V43=68;
...
/* ============================================================
   V43 — AVATARSTAGE AISLADO
   Ningún elemento interno comparte clases con V12–V42.
   ============================================================ */

function getFrameGeometryV43(frameId){
  const row=frameCalibrationsV43.get(frameId)||{};
  return {
...
function ensureTTAvatarV43(host){
  if(!host)return null;
  host.className=[...host.classList].filter(c=>c.startsWith('tt-avatar-')).join(' ');
  if(!host.classList.contains('tt-avatar-v43'))host.classList.add('tt-avatar-v43');

  let stage=host.querySelector(':scope > .tt-avatar-stage-v43');
  if(!stage){
    host.innerHTML='';
    stage=document.createElement('div');
    stage.className='tt-avatar-stage-v43';

    const photo=document.createElement('div');
    photo.className='tt-avatar-photo-v43';
    ...
    frameStage.className='tt-avatar-frame-stage-v43 hidden';
    frameStage.setAttribute('aria-hidden','true');
    const frameImg=document.createElement('img');
    frameImg.className='tt-avatar-frame-img-v43';frameImg.alt='';
    frameStage.appendChild(frameImg);

    const online=document.createElement('span');
    online.className='tt-avatar-online-v43 hidden';
    online.title='Online';online.setAttribute('aria-label','Online');
  ...
  return {
    host,
    stage,
    photo:stage.querySelector(':scope > .tt-avatar-photo-v43'),
    photoImg:stage.querySelector(':scope > .tt-avatar-photo-v43 > img'),
    fallback:stage.querySelector(':scope > .tt-avatar-photo-v43 > span'),
    frameStage:stage.querySelector(':scope > .tt-avatar-frame-stage-v43'),
    frameImg:stage.querySelector(':scope > .tt-avatar-frame-stage-v43 > .tt-avatar-frame-img-v43'),
    online:stage.querySelector(':scope > .tt-avatar-online-v43')
  };
}

function renderTTAvatarV43(host,{frameId='none',geometry=null,photoUrl=null,initials='TT',alt='Foto de perfil',userId=null}={}){
  const p=ensureTTAvatarV43(host);
  if(!p)return;
  ...
  const g=geometry||getFrameGeometryV43(id);
  p.host.dataset.frameId=id;

  const src=id==='none'?'':frameArtUrlV37(id);
  if(src){
    p.frameStage.classList.remove('hidden');
    if(p.frameImg.getAttribute('src')!==src)p.frameImg.src=src;
    p.frameImg.style.width=`${g.frame_scale}%`;
    p.frameImg.style.height=`${g.frame_scale}%`;
    p.frameImg.style.left=`${g.frame_x}%`;
    p.frameImg.style.top=`${g.frame_y}%`;
  }else{
    p.frameStage.classList.add('hidden');
    p.frameImg.removeAttribute('src');
  }
...
function ownFrameSelectionV43(){
  if(v35Flags?.is_test_admin && liveFramePreviewV43?.enabled){
    return {frameId:liveFramePreviewV43.frameId,geometry:liveFramePreviewV43};
  }
  const frameId=frameState?.equipped||'none';
  return {frameId,geometry:getFrameGeometryV43(frameId)};
}

function renderOwnAvatarsV43(){
  if(!profile)return;
  const initials=((profile.first_name?.[0]||'')+(profile.last_name?.[0]||'')).toUpperCase()||'TT';
  const {frameId,geometry}=ownFrameSelectionV43();
  const common={
    frameId,geometry,
  ...
  renderTTAvatarV43($('#homeAvatar'),common);
  renderTTAvatarV43($('#profileAvatar'),common);
}

function refreshOwnVisualsV43(){
  populate();
  renderOwnAvatarsV43();
  renderFrameGallery();
  if(v35Flags?.is_test_admin)setupAdminFrameLabV43();
}

function framePreviewMarkupV43(frameId){
  const initials=((profile?.first_name?.[0]||'')+(profile?.last_name?.[0]||'')).toUpperCase()||'TT';
  const g=getFrameGeometryV43(frameId);
  ...
  return `<div class="tt-frame-preview-v43">
    <div class="tt-frame-preview-photo-v43">${photo}</div>
    ${art?`<div class="tt-frame-preview-stage-v43"><img src="${esc(art)}" alt="" style="width:${g.frame_scale}%;height:${g.frame_scale}%;left:${g.frame_x}%;top:${g.frame_y}%"></div>`:''}
  </div>`;
}

async function loadAvatarCalibrationsV43(){
  try{
    const rows=await getFrameCalibrationsV36();
    frameCalibrationsV43=new Map((rows||[]).map(x=>[x.frame_id,x]));
    renderOwnAvatarsV43();
    renderFrameGallery();
    if(v35Flags?.is_test_admin)setupAdminFrameLabV43();
  }catch(e){console.error('V43 calibraciones',e)}
}

function updateVisibleFrameGeometryV43(frameId,row){
  document.querySelectorAll(`.tt-avatar-v43[data-frame-id="${CSS.escape(frameId)}"]`).forEach(host=>{
    const own=(host.id==='homeAvatar'||host.id==='profileAvatar'||host.id==='adminAvatarPreviewV43');
    if(own && v35Flags?.is_test_admin && liveFramePreviewV43?.enabled && liveFramePreviewV43.frameId===frameId)return;
    const p=ensureTTAvatarV43(host);
    if(!p||p.frameStage.classList.contains('hidden'))return;
    p.frameImg.style.width=`${Number(row.frame_scale??100)}%`;
    p.frameImg.style.height=`${Number(row.frame_scale??100)}%`;
    p.frameImg.style.left=`${Number(row.frame_x??50)}%`;
    p.frameImg.style.top=`${Number(row.frame_y??50)}%`;
  });
}

function startAvatarLiveSyncV43(){
  if(!session?.user?.id)return;
  if(stopAvatarLiveV43)stopAvatarLiveV43().catch?.(()=>{});
  stopAvatarLiveV43=subscribeVisualLiveV36(session.user.id,{
    onProfile:row=>{
      if(!row)return;
      profile={...profile,...row};
      refreshOwnVisualsV43();
    },
    onCosmetics:row=>{
      frameState={...frameState,equipped:row?.equipped_frame_id||null};
      if(!liveFramePreviewV43?.enabled)renderOwnAvatarsV43();
      renderFrameGallery();
    },
    onCalibration:row=>{
      if(!row?.frame_id)return;
      frameCalibrationsV43.set(row.frame_id,row);
      updateVisibleFrameGeometryV43(row.frame_id,row);
      renderFrameGallery();
      if(v35Flags?.is_test_admin && $('#adminFrameSelectV43')?.value===row.frame_id && !liveFramePreviewV43?.enabled){
        setAdminFrameControlsV43(row);
        updateAdminFramePreviewV43(false);
      }
    }
  });
}

function publicFramedAvatarV43(p,frameId='none'){
  const initials=((p?.first_name?.[0]||'')+(p?.last_name?.[0]||'')).toUpperCase()||'TT';
  const g=getFrameGeometryV43(frameId);
  const art=frameId==='none'?'':frameArtUrlV37(frameId);
  const photo=p?.profile_photo_url?`<img src="${esc(p.profile_photo_url)}" alt="Foto de ${esc(p.first_name||'jugador')}">`:`<span>${esc(initials)}</span>`;
  return `<div class="tt-avatar-v43 tt-avatar-public-v43 ${p?.profile_photo_url?'has-photo':''}" data-frame-id="${esc(frameId)}" data-user-id="${esc(p?.id||'')}">
    <div class="tt-avatar-stage-v43">
      <div class="tt-avatar-photo-v43">${photo}</div>
      ${art?`<div class="tt-avatar-frame-stage-v43"><img class="tt-avatar-frame-img-v43" src="${esc(art)}" alt="" aria-hidden="true" style="width:${g.frame_scale}%;height:${g.frame_scale}%;left:${g.frame_x}%;top:${g.frame_y}%"></div>`:'<div class="tt-avatar-frame-stage-v43 hidden"><img class="tt-avatar-frame-img-v43" alt=""></div>'}
      <span class="tt-avatar-online-v43 ${isOnlineV35(p?.id)?'':'hidden'}" title="Online" aria-label="Online"></span>
    </div>
  </div>`;
}

function identityAvatarMarkupV43(p,frameId='none'){
  const initials=((p?.first_name?.[0]||'')+(p?.last_name?.[0]||'')).toUpperCase()||'TT';
  const g=getFrameGeometryV43(frameId);
  const art=frameId==='none'?'':frameArtUrlV37(frameId);
  const photo=p?.profile_photo_url?`<img src="${esc(p.profile_photo_url)}" alt="">`:`<span>${esc(initials)}</span>`;
  return `<div class="tt-avatar-v43 tt-avatar-identity-v43 ${p?.profile_photo_url?'has-photo':''}" data-frame-id="${esc(frameId)}" data-user-id="${esc(p?.id||'')}">
    <div class="tt-avatar-stage-v43">
      <div class="tt-avatar-photo-v43">${photo}</div>
      ${art?`<div class="tt-avatar-frame-stage-v43"><img class="tt-avatar-frame-img-v43" src="${esc(art)}" alt="" aria-hidden="true" style="width:${g.frame_scale}%;height:${g.frame_scale}%;left:${g.frame_x}%;top:${g.frame_y}%"></div>`:'<div class="tt-avatar-frame-stage-v43 hidden"><img class="tt-avatar-frame-img-v43" alt=""></div>'}
      <span class="tt-avatar-online-v43 ${isOnlineV35(p?.id)?'':'hidden'}" title="Online" aria-label="Online"></span>
...
/* ============================================================
   V43 — LABORATORIO: MISMO AVATARSTAGE QUE INICIO/PERFIL
   ============================================================ */

function clampV43(v,min,max){const n=Number(v);return Math.max(min,Math.min(max,Number.isFinite(n)?n:min))}
function fmtV43(v){const n=Number(v);return Number.isInteger(n)?String(n):n.toFixed(1)}
function currentAdminFrameValuesV43(){
  return {
    frame_scale:clampV43($('#adminFrameScaleV43')?.value||100,10,180),
    frame_x:clampV43($('#adminFrameXV43')?.value||50,0,100),
    frame_y:clampV43($('#adminFrameYV43')?.value||50,0,100)
  };
}
function syncAdminControlPairV43(rangeId,numberId,labelId,sourceId){
  const range=$('#'+rangeId),number=$('#'+numberId),label=$('#'+labelId),source=$('#'+sourceId);
  if(!range||!number||!source)return;
  const value=clampV43(source.value,Number(range.min),Number(range.max));
  range.value=value;number.value=value;if(label)label.textContent=`${fmtV43(value)}%`;
}
function setAdminFrameControlsV43(c){
  [
    ['adminFrameScaleV43','adminFrameScaleNumberV43','adminFrameScaleLabelV43',Number(c?.frame_scale??100)],
    ['adminFrameXV43','adminFrameXNumberV43','adminFrameXLabelV43',Number(c?.frame_x??50)],
    ['adminFrameYV43','adminFrameYNumberV43','adminFrameYLabelV43',Number(c?.frame_y??50)]
  ].forEach(([r,n,l,v])=>{if($('#'+r))$('#'+r).value=v;if($('#'+n))$('#'+n).value=v;if($('#'+l))$('#'+l).textContent=`${fmtV43(v)}%`});
}
function setLiveFramePreviewV43(frameId,geometry){
  liveFramePreviewV43={enabled:true,frameId,...geometry};
  renderOwnAvatarsV43();
}
function clearLiveFramePreviewV43(){liveFramePreviewV43=null;renderOwnAvatarsV43()}
function updateAdminFramePreviewV43(propagate=true){
  ...
  const geometry=currentAdminFrameValuesV43();
  const initials=((profile?.first_name?.[0]||'')+(profile?.last_name?.[0]||'')).toUpperCase()||'TT';
  renderTTAvatarV43(preview,{frameId,geometry,photoUrl:profile?.profile_photo_url||null,initials,alt:`Foto de ${profile?.first_name||'jugador'}`,userId:session?.user?.id});
  const frame=(frameState.catalog||[]).find(f=>f.id===frameId);
  if($('#adminFrameNameV43'))$('#adminFrameNameV43').textContent=frame?.name||frameId;
  if(propagate && $('#adminLivePreviewV43')?.checked)setLiveFramePreviewV43(frameId,geometry);
}
function setupAdminFrameLabV43(){
  ...
  const desired=(liveFramePreviewV43?.frameId&&frames.some(f=>f.id===liveFramePreviewV43.frameId))?liveFramePreviewV43.frameId:(frames.some(f=>f.id===select.value)?select.value:(frames.some(f=>f.id===frameState.equipped)?frameState.equipped:frames[0]?.id));
  select.innerHTML=frames.map(f=>`<option value="${esc(f.id)}">${esc(f.name)}</option>`).join('');
  if(desired)select.value=desired;

  if(!select.dataset.boundV43){
    select.dataset.boundV43='1';
    select.addEventListener('change',()=>{setAdminFrameControlsV43(getFrameGeometryV43(select.value));updateAdminFramePreviewV43()});
    [
      ...
      $('#'+r)?.addEventListener('input',()=>{syncAdminControlPairV43(r,n,l,r);updateAdminFramePreviewV43()});
      $('#'+n)?.addEventListener('input',()=>{if($('#'+n).value==='')return;syncAdminControlPairV43(r,n,l,n);updateAdminFramePreviewV43()});
      $('#'+n)?.addEventListener('change',()=>{if($('#'+n).value==='')$('#'+n).value=$('#'+r).value;syncAdminControlPairV43(r,n,l,n);updateAdminFramePreviewV43()});
    });
  }
  const g=(liveFramePreviewV43?.enabled&&liveFramePreviewV43.frameId===select.value)?liveFramePreviewV43:getFrameGeometryV43(select.value);
  setAdminFrameControlsV43(g);updateAdminFramePreviewV43();
}
async function saveAdminFrameV43(){
  const st=$('#adminFrameStatusV43'),frameId=$('#adminFrameSelectV43')?.value;
  if(!frameId)return;
  try{
    const geometry=currentAdminFrameValuesV43();
    const old=frameCalibrationsV43.get(frameId)||{};
    const saved=await saveFrameCalibrationV36(frameId,{...old,inner_diameter:AVATAR_BASE_DIAMETER_V43,photo_scale:100,photo_x:50,photo_y:50,plus_x:82,plus_y:68,...geometry});
    frameCalibrationsV43.set(frameId,saved);
    if($('#adminLivePreviewV43')?.checked)setLiveFramePreviewV43(frameId,geometry);
    updateVisibleFrameGeometryV43(frameId,saved);renderFrameGallery();
    setStatus(st,'Guardado sin salto visual: estos mismos X/Y/escala quedan oficiales.','ok');
  }catch(e){setStatus(st,e.message,'error')}
}
function resetAdminFrameV43(){setAdminFrameControlsV43({frame_scale:100,frame_x:50,frame_y:50});updateAdminFramePreviewV43();setStatus($('#adminFrameStatusV43'),'Restablecido a escala 100 / X 50 / Y 50.','ok')}
...
async function refreshCore(){
  ...
  await loadV35Flags();
  await loadAvatarCalibrationsV43();
  populate();
...
function activateTab(tab){
  ...
  if(tab==='admin')setupAdminFrameLabV43();
...
function renderIdentityShowcase(){
  ...
  box.innerHTML=`
    ${identityAvatarMarkupV43(profile,equippedFrame||'none')}
    ...
    box.innerHTML=`
      <section class="public-profile-v17">
        ...
        <div class="public-profile-v17-head">
          ${publicFramedAvatarV43(p,(cosmetics||publicPrefs)?.equipped_frame_id||'none')}
...
if($('#adminSaveFrameV43'))$('#adminSaveFrameV43').onclick=saveAdminFrameV43;
if($('#adminResetFrameV43'))$('#adminResetFrameV43').onclick=resetAdminFrameV43;
if($('#adminRestoreEquippedV43'))$('#adminRestoreEquippedV43').onclick=()=>{clearLiveFramePreviewV43();if($('#adminLivePreviewV43'))$('#adminLivePreviewV43').checked=false;setStatus($('#adminFrameStatusV43'),'Mostrando el marco realmente equipado.','ok')};
if($('#adminLivePreviewV43'))$('#adminLivePreviewV43').onchange=()=>{$('#adminLivePreviewV43').checked?updateAdminFramePreviewV43():clearLiveFramePreviewV43()};
...
document.addEventListener('click',e=>{
  if(!e.target.closest('.tt-avatar-wrap-v43'))closeProfilePhotoMenu();
});

...
async function refreshCore(){
  ...
  populate();
  await Promise.all([loadRanking(),loadChallenges(),loadMatches(),loadHistory(),loadHomeDashboard(),loadHistoryPage(),loadLiveNotifications(),loadRecommendedRivals(),loadChampionsHall(),loadStatsModeV56(false),loadDurationStatsV59(),loadV28Experience()]);
}
...
async function loadActivityCenter(){
  ...
  try{
    ...
    (notifications||[]).slice(0,30).forEach(n=>{
      items.push({
        kind:n.read_at?'v58-read':'v58-unread',
        icon:notificationIconV58(n.type),
        title:n.title,
        detail:n.body||new Date(n.created_at).toLocaleString('es-UY'),
        attr:`data-notification-v58="${n.id}"`
      });
    });

    if(seasonState){
      const end=new Date(seasonState.ends_at),remaining=Math.max(0,Math.ceil((end-new Date())/86400000));
      items.push({kind:'season',icon:'♛',title:`${seasonState.name||'Temporada actual'} · ${seasonState.position?`#${seasonState.position}`:'sin posición'}`,detail:`${remaining} días restantes`,attr:'data-activity-tab="ranking"'});
    }
    if(userPreferences?.notify_challenges!==false)challenges.filter(c=>c.challenged_id===session.user.id&&c.status==='pending').forEach(c=>{
      items.push({kind:'challenge',icon:'⚔',title:`${c.challenger?.first_name||'Un jugador'} te desafió`,detail:`${matchTypeLabel(c.match_type)} · ${c.match_format==='bo5'?'Bo5':c.match_format==='bo3'?'Bo3':'1 set'}`,attr:'data-activity-tab="play"'});
    });
    if(userPreferences?.notify_confirmations!==false)matches.filter(m=>m.result_status==='awaiting_confirmation'&&m.result_submitted_by!==session.user.id).forEach(m=>{
      const other=m.player1_id===session.user.id?m.player2:m.player1;
      items.push({kind:'confirmation',icon:'✓',title:'Resultado pendiente de confirmar',detail:`vs ${other?.first_name||'Jugador'} ${other?.last_name||''}`,attr:'data-activity-tab="play"'});
    });
    if(userPreferences?.notify_following_activity!==false)feed.slice(0,6).forEach(f=>{
      items.push({kind:'following',icon:f.won?'↑':'↓',title:`${f.followed_first_name} ${f.followed_last_name} ${f.won?'ganó':'perdió'}`,detail:`${f.player_sets}-${f.opponent_sets} vs ${f.opponent_first_name} ${f.opponent_last_name}`,attr:`data-open-player="${f.followed_user_id}"`});
    });

    box.innerHTML=items.length?items.map(i=>`<button class="activity-center-item activity-${i.kind}" ${i.attr} type="button"><span>${i.icon}</span><div><strong>${esc(i.title)}</strong><small>${esc(i.detail)}</small></div><b>›</b></button>`).join(''):'<div class="activity-empty-state"><span>✓</span><strong>Todo al día</strong><small>No tenés acciones pendientes por ahora.</small></div>';
    ...
    $('#matchHistoryList').innerHTML=matches.length?matches.map(m=>{
      ...
      return `<article class="history-match-card ${type}">
        <div class="history-result-band">
          <span>${label}</span>
          <div class="history-result-meta-v59">
            ${m.duration_seconds!==null&&m.duration_seconds!==undefined?`<em>⏱ ${formatDurationV59(m.duration_seconds)}</em>`:''}
...
startMatchClocksV59();
setupPwaV573().catch(err=>console.error('PWA V59:',err));

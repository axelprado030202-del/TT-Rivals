...
import {getFrameCalibrationsV36,saveFrameCalibrationV36,resetFrameCalibrationV36,subscribeVisualLiveV36} from './v36_live.js';
...
const defaultFrameCalibrationV36={
  photo_scale:50,photo_x:50,photo_y:50,frame_scale:100,plus_x:82,plus_y:68
};
...
function framePreviewMarkupV36(frameId){
  ...
  return `<div class="frame-preview avatar-frame-shell ${frameClass(frameId)} ${has?'has-photo':''}" data-frame-calibration-id="${esc(frameId)}" style="${calibrationStyleV36(frameId)}">
    <div class="avatar-photo-mask">
      <img ${has?`src="${esc(profile.profile_photo_url)}"`:''} alt="">
...
function setFrameDraftInputsV36(c){
  const vals={
    ...
    calFrameScaleV36:c.frame_scale,calPlusXV36:c.plus_x,calPlusYV36:c.plus_y
  };
...
function updateFrameCalibratorPreviewV36(){
  ...
  [...preview.classList].filter(c=>c.startsWith('frame-')).forEach(c=>preview.classList.remove(c));
  preview.classList.add(frameClass(id));
  preview.dataset.frameCalibrationId=id;
  applyCalibrationToElementV36(preview,id,draft);
  ...
  const labels={
    calPhotoScaleValueV36:draft.photo_scale,
    calPhotoXValueV36:draft.photo_x,
    calPhotoYValueV36:draft.photo_y,
    calFrameScaleValueV36:draft.frame_scale,
    calPlusXValueV36:draft.plus_x,
    calPlusYValueV36:draft.plus_y
  };
  Object.entries(labels).forEach(([id,v])=>{if($('#'+id))$('#'+id).textContent=`${Number(v).toFixed(Number(v)%1?1:0)}%`});

  // Si el marco calibrado es el equipado, también cambia el perfil real EN VIVO,
  // pero hasta tocar Guardar solo es una previsualización local.
...
function setupFrameCalibratorV36(){
  ...
  if(!select.dataset.boundV36){
    ...
    ['calPhotoScaleV36','calPhotoXV36','calPhotoYV36','calFrameScaleV36','calPlusXV36','calPlusYV36'].forEach(id=>{
...
async function openPublicPlayerProfile(userId){
  ...
  try{
    const [p,showcaseIds,publicPrefs,cosmetics,seasons,records,h2hAdvanced,publicTitles,reliability]=await Promise.all([

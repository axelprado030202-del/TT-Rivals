...
import {getFrameCalibrationsV36,saveFrameCalibrationV36,resetFrameCalibrationV36,subscribeVisualLiveV36,getPublicAdminFlagV37,getPublicAdminIdsV38} from './v36_live.js';
...
let frameCalibrationsV36=new Map(),frameCalDraftV36=null,stopVisualLiveV36=null;
let adminUserIdsV38=new Set(),universalPlayerObserverV38=null;
const AVATAR_BASE_DIAMETER_V41=68;
let adminFrameDraftV41=null;
...
const FRAME_ART_V37={
  ...
  // Elite todavía no tiene un PNG propio en assets; usamos una vista segura del campeón
  // hasta que se incorpore su arte definitivo.
  season_elite:'./assets/frames/season-first.png'
};
...
const defaultFrameCalibrationV36={
  inner_diameter:50,
  photo_scale:92,
  photo_x:50,
  photo_y:50,
...
function effectivePhotoDiameterV38(c){
  // V41: la foto ya NO se adapta a cada PNG.
  // Todos los avatares usan el mismo círculo base.
  return AVATAR_BASE_DIAMETER_V41;
}

function ensureFrameArtLayerV39(el){
  if(!el)return null;
  let art=el.querySelector(':scope > .avatar-frame-art-v39');
  if(!art){
    art=document.createElement('img');
    art.className='avatar-frame-art-v39 hidden';
    art.alt='';
    art.setAttribute('aria-hidden','true');
    el.appendChild(art);
  }
  el.classList.add('avatar-renderer-v39');
  return art;
}

function applyExactAvatarRendererV39(el,frameId,calibration=null){
  if(!el)return;
  const id=frameId||'none';
  const c=calibration||getFrameCalibrationV36(id);
  const effective=effectivePhotoDiameterV38(c);
  const photo=el.querySelector(':scope > .avatar-photo-mask');
  const art=ensureFrameArtLayerV39(el);

  el.dataset.frameCalibrationId=id;
  el.dataset.renderedFrameV39=id;

  // Normalizamos todas las capas al mismo sistema 0–100 usado por el laboratorio.
  if(photo){
    photo.style.width=`${effective}%`;
    photo.style.height=`${effective}%`;
    photo.style.left='50%';
    photo.style.top='50%';
  ...
  if(art){
    if(src){
      ...
      art.style.width=`${Number(c.frame_scale??100)}%`;
      art.style.height=`${Number(c.frame_scale??100)}%`;
      art.style.left=`${Number(c.frame_x??50)}%`;
      art.style.top=`${Number(c.frame_y??50)}%`;
  ...
  // Las variables siguen disponibles para piezas auxiliares, pero ya no dibujan el marco.
  applyCalibrationToElementV36(el,id,c);
}

function applyDraftAcrossOwnInterfaceV39(frameId,draft){
  if(!$('#liveInterfacePreviewV39')?.checked)return;
  [$('#homeAvatar'),$('#profileAvatar')].forEach(el=>applyExactAvatarRendererV39(el,frameId,draft));

  const plus=$('#profilePhotoMenuButton');
  if(plus){
    plus.style.left=`${Number(draft.plus_x??82)}%`;
    plus.style.top=`${Number(draft.plus_y??68)}%`;
  }
}

function restoreOwnEquippedFrameV39(){
  const id=frameState?.equipped||'none';
  const c=getFrameCalibrationV36(id);
  [$('#homeAvatar'),$('#profileAvatar')].forEach(el=>applyExactAvatarRendererV39(el,id,c));
  const plus=$('#profilePhotoMenuButton');
  if(plus){
    plus.style.left=`${Number(c.plus_x??82)}%`;
    plus.style.top=`${Number(c.plus_y??68)}%`;
  }
}

function getFrameCalibrationV36(frameId){
  return frameCalibrationsV36.get(frameId)||defaultFrameCalibrationV36;
}
function calibrationStyleV36(frameId,override=null){
  const c=override||getFrameCalibrationV36(frameId);
  return [
    `--avatar-photo-size:${effectivePhotoDiameterV38(c)}%`,
    `--avatar-inner-diameter:${Number(c.inner_diameter??50)}%`,
    `--avatar-photo-x:${Number(c.photo_x)}%`,
    `--avatar-photo-y:${Number(c.photo_y)}%`,
    `--avatar-frame-scale:${Number(c.frame_scale)/100}`,
    `--avatar-frame-x:${Number(c.frame_x??50)}%`,
    `--avatar-frame-y:${Number(c.frame_y??50)}%`,
    `--avatar-plus-x:${Number(c.plus_x)}%`,
    `--avatar-plus-y:${Number(c.plus_y)}%`
  ].join(';');
}
function applyCalibrationToElementV36(el,frameId,override=null){
  if(!el)return;
  const c=override||getFrameCalibrationV36(frameId);
  el.style.setProperty('--avatar-photo-size',`${effectivePhotoDiameterV38(c)}%`);
  el.style.setProperty('--avatar-inner-diameter',`${Number(c.inner_diameter??50)}%`);
  el.style.setProperty('--avatar-photo-x',`${Number(c.photo_x)}%`);
  el.style.setProperty('--avatar-photo-y',`${Number(c.photo_y)}%`);
  el.style.setProperty('--avatar-frame-scale',String(Number(c.frame_scale)/100));
  el.style.setProperty('--avatar-frame-x',`${Number(c.frame_x??50)}%`);
  el.style.setProperty('--avatar-frame-y',`${Number(c.frame_y??50)}%`);
  el.style.setProperty('--avatar-plus-x',`${Number(c.plus_x)}%`);
  el.style.setProperty('--avatar-plus-y',`${Number(c.plus_y)}%`);
}
function applyAllVisibleFrameCalibrationsV36(){
  $$('[data-frame-calibration-id]').forEach(el=>{
    const id=el.dataset.frameCalibrationId;
    if(el.classList.contains('avatar-renderer-v39'))applyExactAvatarRendererV39(el,id);
    else applyCalibrationToElementV36(el,id);
  });
  restoreOwnEquippedFrameV39();
}
async function loadFrameCalibrationsV36(){
  try{
    const rows=await getFrameCalibrationsV36();
    frameCalibrationsV36=new Map((rows||[]).map(x=>[x.frame_id,x]));
    applyAllVisibleFrameCalibrationsV36();
    renderFrameGallery();
    if(v35Flags?.is_test_admin)setupFrameCalibratorV36();
  }catch(e){console.error('calibraciones V36',e)}
}
function framePreviewMarkupV36(frameId){
  const initials=((profile?.first_name?.[0]||'')+(profile?.last_name?.[0]||'')).toUpperCase()||'TT';
  const has=!!profile?.profile_photo_url;
  const c=getFrameCalibrationV36(frameId);
  const art=frameArtUrlV37(frameId);
  if(frameId==='none'){
    return `<div class="frame-preview-v37 no-frame"><div class="frame-gallery-photo-v37">${has?`<img src="${esc(profile.profile_photo_url)}" alt="">`:`<span>${esc(initials)}</span>`}</div></div>`;
  }
  return `<div class="frame-preview-v37" data-frame-calibration-id="${esc(frameId)}">
    <div class="frame-gallery-photo-v37" style="width:${AVATAR_BASE_DIAMETER_V41}%;height:${AVATAR_BASE_DIAMETER_V41}%;left:50%;top:50%">${has?`<img src="${esc(profile.profile_photo_url)}" alt="">`:`<span>${esc(initials)}</span>`}</div>
    ${art?`<img class="frame-gallery-art-v37" src="${esc(art)}" alt="" style="width:${Number(c.frame_scale)}%;height:${Number(c.frame_scale)}%;left:${Number(c.frame_x??50)}%;top:${Number(c.frame_y??50)}%">`:''}
  </div>`;
}

const framePrecisionControlsV40=[
  {range:'calInnerDiameterV38',number:'calInnerDiameterNumberV40',label:'calInnerDiameterValueV38'},
  {range:'calPhotoScaleV36',number:'calPhotoScaleNumberV40',label:'calPhotoScaleValueV36'},
  {range:'calPhotoXV36',number:'calPhotoXNumberV40',label:'calPhotoXValueV36'},
  {range:'calPhotoYV36',number:'calPhotoYNumberV40',label:'calPhotoYValueV36'},
  {range:'calFrameScaleV36',number:'calFrameScaleNumberV40',label:'calFrameScaleValueV36'},
  {range:'calFrameXV37',number:'calFrameXNumberV40',label:'calFrameXValueV37'},
  {range:'calFrameYV37',number:'calFrameYNumberV40',label:'calFrameYValueV37'},
  {range:'calPlusXV36',number:'calPlusXNumberV40',label:'calPlusXValueV36'},
  {range:'calPlusYV36',number:'calPlusYNumberV40',label:'calPlusYValueV36'}
];

function clampPrecisionV40(value,input){
...
function formatPrecisionV40(value){
  const n=Number(value);
  if(!Number.isFinite(n))return '0';
  return Number.isInteger(n)?String(n):n.toFixed(1);
}
function syncPrecisionPairV40(sourceId){
  const item=framePrecisionControlsV40.find(x=>x.range===sourceId||x.number===sourceId);
  if(!item)return;
  const range=$('#'+item.range),number=$('#'+item.number),label=$('#'+item.label);
  if(!range||!number)return;
  const source=$('#'+sourceId);
  const value=clampPrecisionV40(source.value,source);
  range.value=value;
  number.value=value;
  if(label)label.textContent=`${formatPrecisionV40(value)}%`;
}
function setPrecisionValueV40(rangeId,value){
  const item=framePrecisionControlsV40.find(x=>x.range===rangeId);
  if(!item)return;
  const range=$('#'+item.range),number=$('#'+item.number),label=$('#'+item.label);
  if(!range||!number)return;
  const v=clampPrecisionV40(value,range);
  range.value=v;
  number.value=v;
  if(label)label.textContent=`${formatPrecisionV40(v)}%`;
}
function bindPrecisionControlsV40(){
  framePrecisionControlsV40.forEach(item=>{
    const range=$('#'+item.range),number=$('#'+item.number);
    if(!range||!number)return;
    if(range.dataset.precisionBoundV40)return;
    range.dataset.precisionBoundV40='1';

    range.addEventListener('input',()=>{
      syncPrecisionPairV40(item.range);
      updateFrameCalibratorPreviewV36();
    });
    number.addEventListener('input',()=>{
      if(number.value==='')return;
      syncPrecisionPairV40(item.number);
      updateFrameCalibratorPreviewV36();
    });
...
function readFrameDraftV36(){
  return {
    inner_diameter:Number($('#calInnerDiameterV38')?.value||50),
    photo_scale:Number($('#calPhotoScaleV36')?.value||92),
    photo_x:Number($('#calPhotoXV36')?.value||50),
    photo_y:Number($('#calPhotoYV36')?.value||50),
    frame_scale:Number($('#calFrameScaleV36')?.value||100),
    frame_x:Number($('#calFrameXV37')?.value||50),
    frame_y:Number($('#calFrameYV37')?.value||50),
    plus_x:Number($('#calPlusXV36')?.value||82),
    plus_y:Number($('#calPlusYV36')?.value||68)
  };
}
function setFrameDraftInputsV36(c){
  const vals={
    calInnerDiameterV38:c.inner_diameter??50,
    calPhotoScaleV36:c.photo_scale??92,
    calPhotoXV36:c.photo_x??50,
    calPhotoYV36:c.photo_y??50,
    calFrameScaleV36:c.frame_scale??100,
    calFrameXV37:c.frame_x??50,
    calFrameYV37:c.frame_y??50,
    calPlusXV36:c.plus_x??82,
    calPlusYV36:c.plus_y??68
  };
  Object.entries(vals).forEach(([id,v])=>setPrecisionValueV40(id,v));
  updateFrameCalibratorPreviewV36();
}
function updateFrameCalibratorPreviewV36(){
  const select=$('#frameCalibrationSelectV36');
  const preview=$('#frameCalibrationPreviewV36');
  if(!select||!preview)return;
  const id=select.value;
  const draft=readFrameDraftV36();
  frameCalDraftV36={frame_id:id,...draft};

  // V37: laboratorio AISLADO. No usa pseudo-elementos ni clases CSS legacy.
  // Cada capa tiene su propio elemento real y no puede deformar a las otras.
  const photo=$('#adminFramePhotoLayerV37');
  const art=$('#adminFrameArtV37');
  const plus=$('#frameCalibrationFakePlusV36');
  const img=$('#frameCalibrationPreviewImgV36');
  const fallback=$('#frameCalibrationPreviewFallbackV36');

  if(photo){
    const effectivePhoto=effectivePhotoDiameterV38(draft);
    photo.style.width=`${effectivePhoto}%`;
    photo.style.height=`${effectivePhoto}%`;
    photo.style.left=`${draft.photo_x}%`;
    photo.style.top=`${draft.photo_y}%`;
  }
  if(art){
    const src=frameArtUrlV37(id);
    art.src=src;
    art.classList.toggle('hidden',!src);
    art.style.width=`${draft.frame_scale}%`;
    art.style.height=`${draft.frame_scale}%`;
    art.style.left=`${draft.frame_x}%`;
    art.style.top=`${draft.frame_y}%`;
  }
  if(plus){plus.style.left=`${draft.plus_x}%`;plus.style.top=`${draft.plus_y}%`}
  ...
  const name=(frameState.catalog||[]).find(x=>x.id===id)?.name||id;
  if($('#frameCalibrationNameV36'))$('#frameCalibrationNameV36').textContent=name;
  const labels={
    calInnerDiameterValueV38:draft.inner_diameter,
    calPhotoScaleValueV36:draft.photo_scale,calPhotoXValueV36:draft.photo_x,calPhotoYValueV36:draft.photo_y,
    calFrameScaleValueV36:draft.frame_scale,calFrameXValueV37:draft.frame_x,calFrameYValueV37:draft.frame_y,
    calPlusXValueV36:draft.plus_x,calPlusYValueV36:draft.plus_y
  };
  Object.entries(labels).forEach(([key,v])=>{if($('#'+key))$('#'+key).textContent=`${Number(v).toFixed(Number(v)%1?1:0)}%`});

  // V39 WYSIWYG: el mismo draft se dibuja en Inicio y Perfil con el MISMO motor,
  // incluso si el marco seleccionado todavía no está equipado.
  applyDraftAcrossOwnInterfaceV39(id,draft);
}
function setupFrameCalibratorV36(){
  if(!v35Flags?.is_test_admin||!$('#frameCalibrationSelectV36'))return;
  const select=$('#frameCalibrationSelectV36');
  const frames=(frameState.catalog||[]).filter(f=>f.id!=='none');
  const current=select.value;
  select.innerHTML=frames.map(f=>`<option value="${esc(f.id)}">${esc(f.name)}</option>`).join('');
  const desired=(current&&frames.some(f=>f.id===current))?current:(frameState.equipped&&frames.some(f=>f.id===frameState.equipped)?frameState.equipped:frames[0]?.id);
  if(desired)select.value=desired;

  const c=getFrameCalibrationV36(select.value);
  setFrameDraftInputsV36(c);

  bindPrecisionControlsV40();
  if(!select.dataset.boundV36){
    select.dataset.boundV36='1';
    select.addEventListener('change',()=>setFrameDraftInputsV36(getFrameCalibrationV36(select.value)));
    bindPrecisionControlsV40();
  }
}
function refreshOwnVisualsV36(){
  populate();
  renderFrameGallery();
  applyAllVisibleFrameCalibrationsV36();
  if(v35Flags?.is_test_admin)setupFrameCalibratorV36();
}
function startVisualLiveSyncV36(){
  if(!session?.user?.id)return;
  if(stopVisualLiveV36)stopVisualLiveV36().catch?.(()=>{});
  stopVisualLiveV36=subscribeVisualLiveV36(session.user.id,{
    onProfile:row=>{
      if(!row)return;
      profile={...profile,...row};
      refreshOwnVisualsV36();
    },
    onCosmetics:row=>{
    ...
    onCalibration:row=>{
      ...
      frameCalibrationsV36.set(row.frame_id,row);
      renderFrameGallery();
      const selected=$('#frameCalibrationSelectV36')?.value;
      const previewing=!!($('#liveInterfacePreviewV39')?.checked && selected);
      if(!previewing)applyAllVisibleFrameCalibrationsV36();
      if(v35Flags?.is_test_admin && selected!==row.frame_id){
        setupFrameCalibratorV36();
      }
      if(v35Flags?.is_test_admin && $('#adminFrameSelectV41')){
        const adminSelected=$('#adminFrameSelectV41').value;
        if(adminSelected===row.frame_id && !adminFrameDraftV41){
          setAdminFrameControlsV41(row);
        }
      }
    }
  });
...
function currentAdminFrameValuesV41(){
  return {
    frame_scale:clampV41($('#adminFrameScaleV41')?.value||100,10,180),
    frame_x:clampV41($('#adminFrameXV41')?.value||50,0,100),
    frame_y:clampV41($('#adminFrameYV41')?.value||50,0,100)
  };
}
function syncAdminControlPairV41(rangeId,numberId,labelId,sourceId){
  const range=$('#'+rangeId),number=$('#'+numberId),label=$('#'+labelId);
  if(!range||!number)return;
  const source=$('#'+sourceId);
  const value=clampV41(source?.value,Number(range.min),Number(range.max));
  range.value=value;
  number.value=value;
  if(label)label.textContent=`${fmtV41(value)}%`;
}
function setAdminFrameControlsV41(c){
  const vals=[
    ['adminFrameScaleV41','adminFrameScaleNumberV41','adminFrameScaleLabelV41',Number(c?.frame_scale??100)],
    ['adminFrameXV41','adminFrameXNumberV41','adminFrameXLabelV41',Number(c?.frame_x??50)],
    ['adminFrameYV41','adminFrameYNumberV41','adminFrameYLabelV41',Number(c?.frame_y??50)]
  ];
  vals.forEach(([r,n,l,v])=>{
    if($('#'+r))$('#'+r).value=v;
    if($('#'+n))$('#'+n).value=v;
    if($('#'+l))$('#'+l).textContent=`${fmtV41(v)}%`;
  });
  updateAdminFramePreviewV41();
}
function applyAdminDraftToOwnInterfaceV41(frameId,draft){
  if(!$('#adminLivePreviewV41')?.checked)return;
  const base=getFrameCalibrationV36(frameId);
  const merged={...base,...draft,photo_x:50,photo_y:50};
  [$('#homeAvatar'),$('#profileAvatar')].forEach(el=>applyExactAvatarRendererV39(el,frameId,merged));
}
function updateAdminFramePreviewV41(){
  if(!v35Flags?.is_test_admin)return;
  const select=$('#adminFrameSelectV41');
  const preview=$('#adminAvatarPreviewV41');
  if(!select||!preview)return;

  const frameId=select.value;
  const draft=currentAdminFrameValuesV41();
  adminFrameDraftV41={frame_id:frameId,...draft};

  const base=getFrameCalibrationV36(frameId);
  const merged={...base,...draft,photo_x:50,photo_y:50};
  applyExactAvatarRendererV39(preview,frameId,merged);

  const has=!!profile?.profile_photo_url;
  const img=$('#adminAvatarPreviewImgV41');
  const fallback=$('#adminAvatarPreviewFallbackV41');
  if(img){
    if(has){img.src=profile.profile_photo_url;preview.classList.add('has-photo')}
    else{img.removeAttribute('src');preview.classList.remove('has-photo')}
  }
  if(fallback)fallback.textContent=((profile?.first_name?.[0]||'')+(profile?.last_name?.[0]||'')).toUpperCase()||'TT';

  const frame=(frameState.catalog||[]).find(f=>f.id===frameId);
  if($('#adminFrameNameV41'))$('#adminFrameNameV41').textContent=frame?.name||frameId;

  applyAdminDraftToOwnInterfaceV41(frameId,draft);
}
function setupAdminFrameLabV41(){
  if(!v35Flags?.is_test_admin||!$('#adminFrameSelectV41'))return;
  const select=$('#adminFrameSelectV41');
  const frames=(frameState.catalog||[]).filter(f=>f.id!=='none'&&frameArtUrlV37(f.id));
  const previous=select.value;

  select.innerHTML=frames.map(f=>`<option value="${esc(f.id)}">${esc(f.name)}</option>`).join('');
  const desired=frames.some(f=>f.id===previous)?previous:
    (frames.some(f=>f.id===frameState.equipped)?frameState.equipped:frames[0]?.id);
  if(desired)select.value=desired;

  if(!select.dataset.boundV41){
    select.dataset.boundV41='1';
    select.addEventListener('change',()=>setAdminFrameControlsV41(getFrameCalibrationV36(select.value)));

    const pairs=[
      ['adminFrameScaleV41','adminFrameScaleNumberV41','adminFrameScaleLabelV41'],
      ['adminFrameXV41','adminFrameXNumberV41','adminFrameXLabelV41'],
      ['adminFrameYV41','adminFrameYNumberV41','adminFrameYLabelV41']
    ];
    pairs.forEach(([rangeId,numberId,labelId])=>{
      const range=$('#'+rangeId),number=$('#'+numberId);
      range?.addEventListener('input',()=>{
        syncAdminControlPairV41(rangeId,numberId,labelId,rangeId);
        updateAdminFramePreviewV41();
      });
      number?.addEventListener('input',()=>{
        if(number.value==='')return;
        syncAdminControlPairV41(rangeId,numberId,labelId,numberId);
        updateAdminFramePreviewV41();
      });
      number?.addEventListener('change',()=>{
        if(number.value==='')number.value=range.value;
  ...
  setAdminFrameControlsV41(getFrameCalibrationV36(select.value));
}
async function saveAdminFrameV41(){
  const st=$('#adminFrameStatusV41');
  const frameId=$('#adminFrameSelectV41')?.value;
  if(!frameId)return;
  try{
    const old=getFrameCalibrationV36(frameId);
    const draft=currentAdminFrameValuesV41();
    // Los campos históricos de foto se conservan en DB por compatibilidad,
    // pero V41 ya no los utiliza: la foto es un círculo base fijo.
    const saved=await saveFrameCalibrationV36(frameId,{
      ...old,
      inner_diameter:AVATAR_BASE_DIAMETER_V41,
      photo_scale:100,
      photo_x:50,
      photo_y:50,
      ...draft
    });
    frameCalibrationsV36.set(frameId,saved);
    adminFrameDraftV41={frame_id:frameId,...draft};
    updateAdminFramePreviewV41();
    renderFrameGallery();
    setStatus(st,'Marco guardado. El perfil usa exactamente esta posición y escala.','ok');
  }catch(e){setStatus(st,e.message,'error')}
}
async function resetAdminFrameV41(){
  const frameId=$('#adminFrameSelectV41')?.value;
  if(!frameId)return;
  setAdminFrameControlsV41({...getFrameCalibrationV36(frameId),frame_scale:100,frame_x:50,frame_y:50});
  setStatus($('#adminFrameStatusV41'),'Vista restablecida a centro 50/50 y escala 100%. Guardá si querés hacerla oficial.','ok');
}
...
function renderFrameGallery(){
  ...
  box.innerHTML=rows.map(f=>{
    ...
    const cardTier=f.id==='season_first'?'season-card-first':f.id==='season_second'?'season-card-second':f.id==='season_third'?'season-card-third':'';
    return `<article class="frame-card ${cardTier} ${available?'unlocked':'locked'} ${equipped?'equipped':''}" data-frame-id="${esc(f.id)}">
      ${framePreviewMarkupV36(f.id)}
...
function publicFramedAvatarV34(p,frameId='none'){
  const has=!!p?.profile_photo_url;
  const initials=((p?.first_name?.[0]||'')+(p?.last_name?.[0]||'')).toUpperCase()||'TT';
  const c=getFrameCalibrationV36(frameId);
  const art=frameId==='none'?'':frameArtUrlV37(frameId);
  return `<div class="public-avatar-frame avatar-frame-shell avatar-renderer-v39 ${has?'has-photo':''}" data-has-photo="${has?'true':'false'}" data-frame-calibration-id="${esc(frameId)}">
    <div class="avatar-photo-mask" style="width:${AVATAR_BASE_DIAMETER_V41}%;height:${AVATAR_BASE_DIAMETER_V41}%;left:50%;top:50%;transform:translate(-50%,-50%)">
      ${has?`<img src="${esc(p.profile_photo_url)}" alt="Foto de ${esc(p.first_name||'jugador')}">`:'<img alt="">'}
      <span>${esc(initials)}</span>
    </div>
    ${art?`<img class="avatar-frame-art-v39" src="${esc(art)}" alt="" aria-hidden="true" style="width:${Number(c.frame_scale??100)}%;height:${Number(c.frame_scale??100)}%;left:${Number(c.frame_x??50)}%;top:${Number(c.frame_y??50)}%;transform:translate(-50%,-50%)">`:''}
  </div>`;
}
...
async function loadV35Flags(){
  ...
  $('#adminNavItemV41')?.classList.toggle('hidden',!v35Flags.is_test_admin);
  document.body.classList.toggle('is-admin-v41',!!v35Flags.is_test_admin);
...
function populate(){
  ...
  const equippedFrameId=frameState?.equipped||'none';
  const equippedFrameClass=frameClass(equippedFrameId);
  if(avatar){
    ...
    avatar.dataset.frameCalibrationId=equippedFrameId;
    applyExactAvatarRendererV39(avatar,equippedFrameId);
  }
  ...
  if(profileAvatar&&profileAvatarImg&&profileAvatarFallback){
    ...
    profileAvatar.dataset.frameCalibrationId=equippedFrameId;
    applyExactAvatarRendererV39(profileAvatar,equippedFrameId);
...
async function refreshCore(){
  ...
  await loadV35Flags();
  await loadFrameCalibrationsV36();
  populate();
  await Promise.all([loadRanking(),loadChallenges(),loadMatches(),loadHistory(),loadHomeDashboard(),loadHistoryPage(),loadLiveNotifications(),loadRecommendedRivals(),loadChampionsHall(),loadPersonalRecords(),loadV28Experience()]);
}
...
function activateTab(tab){
  ...
  if(tab==='settings'){populateSettingsUI();renderFrameGallery();loadV35Flags().then(()=>{renderFrameGallery();renderAchievements();});}
  if(tab==='admin')setupAdminFrameLabV41();
    ...
    if(!liveMatchStatusPrimed){
      ...
      for(const m of matches){
        ...
        if(m.result_status==='confirmed'&&previous&&previous!=='confirmed'&&!postMatchShownIds.has(id)){
          ...
          setTimeout(()=>showPostMatch({matchId:id,won:m.winner_id===session.user.id,oldRating:null,newRating:null}),40);
        }
    ...
    box.innerHTML=`
      <section class="public-profile-v17">
        ...
        <div class="public-profile-v17-head">
          ${publicFramedAvatarV34(p,(cosmetics||publicPrefs)?.equipped_frame_id||'none')}
...
async function searchTeamPlayerV32(input){
  ...
  teamSearchTimerV32=setTimeout(async()=>{
    try{
      ...
      // V32 FIX2 - MODO PRUEBA TEMPORAL:
      // permitimos reutilizar el mismo usuario en cualquier casillero.
...
function openTeamTournamentBuilderV32(){
  ...
  $('#teamTournamentBuilderV32').classList.remove('hidden');
  $('#teamTournamentBuilderV32').classList.add('team-test-mode-v32');
...
function renderTeamTournamentDetailV32(){
  ...
  if(['completed','draw'].includes(t.status)){
    ...
    summary.innerHTML=`<div class="section-title-row">
      ...
      ${t.test_mode?'<div class="team-elo-test-v33">🧪 Modo prueba: los resultados no modificaron Elo.</div>':''}
    ...
    box.innerHTML=rows.length?rows.map(t=>{
      ...
      return `<button class="team-history-card-v33" type="button" data-open-team-tournament="${t.tournament_id}">
        ...
        <div class="team-history-meta-v33">
          ...
          <small>${t.completed_at?new Date(t.completed_at).toLocaleDateString('es-UY'):''}${t.test_mode?' · 🧪 prueba':''}</small>
        </div>
      </button>`;
...
function showTeamEloV33(){
  ...
  $('#teamEloTestNoticeV33').classList.toggle('hidden',!t.test_mode);
...
if($('#liveInterfacePreviewV39'))$('#liveInterfacePreviewV39').onchange=()=>{
  if($('#liveInterfacePreviewV39').checked){
    const id=$('#frameCalibrationSelectV36')?.value;
    if(id)applyDraftAcrossOwnInterfaceV39(id,readFrameDraftV36());
  }else{
    restoreOwnEquippedFrameV39();
  }
};
...
if($('#adminSaveFrameV41'))$('#adminSaveFrameV41').onclick=saveAdminFrameV41;
if($('#adminResetFrameV41'))$('#adminResetFrameV41').onclick=resetAdminFrameV41;
if($('#adminRestoreEquippedV41'))$('#adminRestoreEquippedV41').onclick=()=>{
  restoreOwnEquippedFrameV39();
  setStatus($('#adminFrameStatusV41'),'Mostrando nuevamente el marco realmente equipado.','ok');
};
if($('#adminLivePreviewV41'))$('#adminLivePreviewV41').onchange=()=>{
  if($('#adminLivePreviewV41').checked)updateAdminFramePreviewV41();
  else restoreOwnEquippedFrameV39();
};

if($('#saveFrameCalibrationV36'))$('#saveFrameCalibrationV36').onclick=async()=>{
  const st=$('#frameCalibrationStatusV36');
  const frameId=$('#frameCalibrationSelectV36')?.value;
  if(!frameId)return;
  try{
    setStatus(st,'Guardando calibración…');
    const saved=await saveFrameCalibrationV36(frameId,readFrameDraftV36());
    frameCalibrationsV36.set(frameId,saved);
    renderFrameGallery();
    if($('#liveInterfacePreviewV39')?.checked){
      applyDraftAcrossOwnInterfaceV39(frameId,saved);
    }else{
      applyAllVisibleFrameCalibrationsV36();
    }
    setStatus(st,'Calibración publicada. Perfil e Inicio ya están usando exactamente estas coordenadas.','ok');
  }catch(e){setStatus(st,e.message,'error')}
};
if($('#resetFrameCalibrationV36'))$('#resetFrameCalibrationV36').onclick=async()=>{
  const st=$('#frameCalibrationStatusV36');
  const frameId=$('#frameCalibrationSelectV36')?.value;
  if(!frameId)return;
  try{
    const saved=await resetFrameCalibrationV36(frameId);
    frameCalibrationsV36.set(frameId,saved);
    setFrameDraftInputsV36(saved);
    renderFrameGallery();
    if(!$('#liveInterfacePreviewV39')?.checked)applyAllVisibleFrameCalibrationsV36();
    setStatus(st,'Valores originales restaurados y aplicados en vivo.','ok');
  }catch(e){setStatus(st,e.message,'error')}
};
...
$('#matchResultForm').onsubmit=async e=>{
  ...
  try{
    ...
    closeMatchModal();
    await Promise.all([loadMatches(),loadV28Experience()]);
...
document.addEventListener('click',async e=>{
  ...
  if(frameBtn){
    ...
    try{
      ...
      frameState={...frameState,equipped:chosen==='none'?null:chosen};
      refreshOwnVisualsV36();
      setStatus($('#settingsStatus'),'Marco actualizado en vivo.','ok');
...
$('#teamTournamentFormV32').onsubmit=async e=>{
  ...
  // V32 FIX2 - MODO PRUEBA TEMPORAL:
  // durante las pruebas se permite repetir el mismo usuario en varios lugares.
  const pairIds={

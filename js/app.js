...
import {getMyProfile,getMyRatings,completeSportsProfile,getClubsV47,ensureClubV47,getRanking,searchPlayers,getRatingHistory,getRankTiers,setProfilePhotoUrl,uploadProfilePhoto,deleteProfilePhotoByUrl} from './profile.js';
...
function showView(id){authShell.classList.remove('hidden');mainApp.classList.add('hidden');views.forEach(v=>v.classList.toggle('active',v.id===id));window.scrollTo(0,0);if(id==='sportsProfileView')loadAvailableClubsV47().catch(err=>console.warn('Clubes V47:',err))}
...
function activateTab(tab){
  ...
  if(tab==='admin')setupAdminFrameLabV44();
...
function normalizeClubNameClientV47(value=''){
  return String(value).trim().replace(/\s+/g,' ');
}
async function loadAvailableClubsV47(preferred=''){
  const select=$('#clubName');
  if(!select)return;
  const previous=preferred||select.value||'';
  try{
    availableClubsV47=await getClubsV47();
    const options=[
      '<option value="">Seleccioná</option>',
      '<option value="N/A">N/A — No pertenezco a un club</option>',
      ...(availableClubsV47||[]).map(c=>`<option value="${esc(c.name)}">${esc(c.name)}</option>`),
      '<option value="Otro">Otro</option>'
    ];
    select.innerHTML=options.join('');
    if(previous&&[...select.options].some(o=>o.value===previous))select.value=previous;
    const hint=$('#clubDirectoryHintV47');
    if(hint)hint.textContent=availableClubsV47.length
      ?`${availableClubsV47.length} club${availableClubsV47.length===1?'':'es'} disponible${availableClubsV47.length===1?'':'s'} en TT Rivals.`
      :'Si alguien registra un club nuevo, aparecerá acá automáticamente.';
  }catch(err){
    console.error('loadAvailableClubsV47',err);
    const hint=$('#clubDirectoryHintV47');
...
$('#clubName').onchange=()=>{const c=$('#clubName').value==='Otro';$('#customClubWrap').classList.toggle('hidden',!c);$('#customClubName').required=c;if(c)setTimeout(()=>$('#customClubName')?.focus(),80)};
...
$('#sportsProfileForm').onsubmit=async e=>{
  ...
  let finalClub=selectedClub==='Otro'?normalizeClubNameClientV47($('#customClubName').value):selectedClub;
  ...
  if(selectedClub==='Otro'&&finalClub.length<2){
    return setStatus(status,'Escribí el nombre del club.','error');
  }
  ...
  try{
    ...
    // V47: un club escrito en “Otro” se registra una sola vez y devuelve
    // el nombre canónico. Los próximos usuarios lo verán en el selector.
    if(selectedClub==='Otro'){
      setStatus(status,'Registrando club…');
      finalClub=await ensureClubV47(finalClub);
    }
...
$('#competitiveProfileSettingsForm').onsubmit=async e=>{
  ...
  try{
    ...
    if(payload.club_name&&payload.club_name!=='N/A')payload.club_name=await ensureClubV47(payload.club_name);
...
document.addEventListener('click',e=>{
  if(!e.target.closest('.tt-avatar-wrap-v44'))closeProfilePhotoMenu();
});

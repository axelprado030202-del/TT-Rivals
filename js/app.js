...
function achievementGroupGlyph(group=''){
  return ({
    'Partidos':'⚔',
    'Victorias':'♛',
    'Rachas':'🔥',
...
function achievementEmblemHtml(a,compact=false){
  const numeric=/^\d+$/.test(String(a.icon||''))?a.icon:null;
  const glyph=numeric||achievementGroupGlyph(a.group);
  return `<span class="achievement-emblem rarity-${a.rarity||'common'} group-${String(a.group||'').toLowerCase().replaceAll('ó','o').replaceAll(' ','-')} ${compact?'compact':''}">
    <span class="achievement-emblem-ring"></span>
    <span class="achievement-emblem-core">${esc(glyph)}</span>
    <span class="achievement-emblem-spark spark-a"></span>
    <span class="achievement-emblem-spark spark-b"></span>
  </span>`;
}

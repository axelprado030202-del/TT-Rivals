...
function renderDailyMissionsV101(daily=null,errorMessage=''){
  ...
  if(!missions)return;
  if(errorMessage){
    missions.innerHTML=`<article class="ui101-mission-error"><div><strong>No pudimos cargar las misiones.</strong><small>${esc(errorMessage||'La conexión respondió con un error. Podés reintentar sin recargar TT Rivals.')}</small></div><button id="retryDailyMissionsV101" type="button">↻ Reintentar</button></article>`;
    return;
  }
  if(!daily){
    missions.innerHTML='<article class="ui101-mission-error"><div><strong>Misiones temporalmente no disponibles.</strong><small>El resto de TT Rivals sigue funcionando con normalidad.</small></div><button id="retryDailyMissionsV101" type="button">↻ Reintentar</button></article>';
    return;
  }
  ...
  if(!all.length){
    missions.innerHTML='<article class="ui101-mission-error"><div><strong>Todavía no hay misiones asignadas.</strong><small>TT Rivals intentará generarlas nuevamente.</small></div><button id="retryDailyMissionsV101" type="button">↻ Generar misiones</button></article>';
    return;
  }

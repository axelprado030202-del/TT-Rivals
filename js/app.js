...
function renderV28Dashboard(){
  ...
  if(missions){
    const daily=d.daily_missions_v101||null;
    if(daily){
      const all=daily.missions||[];
      const active=all.filter(m=>!m.completed);
      const reset=Number.isFinite(Date.parse(daily.reset_at))?new Date(daily.reset_at):null;
      const left=reset?Math.max(0,reset-Date.now()):0;
      const hours=Math.floor(left/3600000),mins=Math.floor((left%3600000)/60000);
      if(daily.all_completed){
        ...
        missions.innerHTML=`<div class="v101-mission-cycle-head"><span>${Number(daily.completed||0)} / ${Number(daily.total||3)} completadas</span><small data-v101-mission-reset="${esc(daily.reset_at||'')}">Renuevan en ${hours} h ${mins} min</small></div>${active.map(m=>{
          ...
          return `<article class="v28-mission v101-mission-${esc(m.difficulty||'easy')}">
            <div><span>${diff}</span><strong>${esc(m.title)}</strong><small>${esc(m.description||'')} · +${m.reward} XP</small></div>
      ...
      missions.innerHTML=(d.missions||[]).filter(m=>Number(m.progress)<Number(m.goal)).map(m=>{
        const pct=Math.min(100,(Number(m.progress||0)/Math.max(1,Number(m.goal||1)))*100);
        return `<article class="v28-mission"><div><span>HOY</span><strong>${esc(m.title)}</strong><small>+${m.reward} XP</small></div><b>${m.progress}/${m.goal}</b><div class="v28-mission-track"><i style="width:${pct}%"></i></div></article>`;
      }).join('');
    }
  }

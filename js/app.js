...
let userPreferences=null,frameState={catalog:[],unlocks:[],equipped:null},seasonState=null,percentileState={};
let historyModeFilter='all',historyResultFilter='all';
...
async function loadExperienceSettings(){
  ...
  try{
    [userPreferences,frameState,seasonState,percentileState]=await Promise.all([
      getPreferences(),
...
function lockPageScroll(lock=true){
  ...
  if(lock){
    ...
    if(!body.classList.contains('modal-open'))return;
    const y=Math.abs(parseInt(body.dataset.scrollY||'0',10));
...
async function refreshCore(){
  ...
  populate();
  await Promise.all([loadRanking(),loadChallenges(),loadMatches(),loadHistory(),loadHomeDashboard(),loadHistoryPage(),loadLiveNotifications(),loadRecommendedRivals()]);
}
...
function showPostMatch({won,oldRating,newRating,opponentName='Rival'}){
  const delta=newRating-oldRating;
  const oldRank=rankForRating(oldRating),newRank=rankForRating(newRating);
  const rankedUp=oldRank!==newRank&&newRating>oldRating;
  $('#postMatchContent').innerHTML=`
    <div class="post-match-result ${won?'victory':'defeat'}">
      <span>${won?'VICTORIA':'PARTIDO CONFIRMADO'}</span>
      <h2>${delta>=0?'+':''}${delta} Elo</h2>
      <p>${oldRating} → <strong>${newRating}</strong></p>
    </div>
    ${rankedUp?`<div class="post-rank-up"><small>ASCENSO</small><strong>${esc(newRank)} desbloqueado</strong><span>También desbloqueaste el Marco ${esc(newRank)}.</span></div>`:''}
...
async function openPublicPlayerProfile(userId){
  ...
  try{
    const [p,showcaseIds,publicPrefs,cosmetics]=await Promise.all([
...
document.addEventListener('click',async e=>{
  ...
  const confirm=e.target.closest('[data-confirm-match]');if(confirm){
    ...
    try{
      ...
      showPostMatch({won,oldRating:before,newRating:after});
...
$('#closeReviewModal').onclick=()=>$('#reviewModal').classList.add('hidden');syncModalScrollLock();

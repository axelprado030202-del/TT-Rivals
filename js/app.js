...
async function renderTournamentBracket(){
  ...
  if(!selectedTournament.started_at){
    ...
    try{
      ...
      box.innerHTML=`<section class="t30-lobby">
        ...
        <div class="t30-lobby-people">
          ${participants.length?participants.map(p=>`<div><span>${p.photo?`<img src="${esc(p.photo)}" alt="">`:`${esc((p.name||'TT').slice(0,2).toUpperCase())}`}</span><strong>${esc(p.name||p.username||'Jugador')}</strong><small>@${esc(p.username||'')}</small></div>`).join(''):'<p class="loading-row">Todavía no hay participantes.</p>'}
        </div>
        ${lobby.is_host
          ?`<div class="t30-host-actions"><button class="btn btn-start" type="button" data-start-tournament-v30="${selectedTournament.id}">INICIAR TORNEO</button><small>Al iniciar se cierran las inscripciones, se sortean las parejas si es 2vs2 y se generan los grupos automáticamente.</small></div>`
          :lobby.already_joined
            ?'<div class="t30-waiting-host">✓ Estás inscripto. Esperando a que el organizador inicie el torneo.</div>'
            :''}
      </section>`;
    }catch(err){
      console.error(err);
  ...
  box.innerHTML=html||`<div class="t30-structure-error">
    <strong>No se generó la estructura del torneo.</strong>
    <span>Ya no dejamos esta pantalla cargando indefinidamente.</span>
    <button type="button" data-retry-tournament-v30>Reintentar</button>
  </div>`;
...
$('#tournamentBracket').onclick=async e=>{
  ...
  if(start){
    if(!confirm('Se cerrarán las inscripciones y se generará el torneo. ¿Continuar?'))return;
    try{

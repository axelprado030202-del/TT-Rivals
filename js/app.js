...
function applyAllVisibleFrameCalibrationsV36(){
  $$('[data-frame-calibration-id]').forEach(el=>applyCalibrationToElementV36(el,el.dataset.frameCalibrationId));
  const own=frameState?.equipped||'none';
  [$('#homeAvatar'),$('#profileAvatar')].forEach(el=>{
    if(el){
      el.dataset.frameCalibrationId=own;
      applyCalibrationToElementV36(el,own);
    }
  });
  const plus=$('#profilePhotoMenuButton');
  if(plus&&$('#homeAvatar')){
    const c=getFrameCalibrationV36(own);
    plus.style.left=`${c.plus_x}%`;
    plus.style.top=`${c.plus_y}%`;
  }
}
...
function setFrameDraftInputsV36(c){
  const vals={
    ...
    calPhotoScaleV36:c.photo_scale??92,calPhotoXV36:c.photo_x,calPhotoYV36:c.photo_y,
    calFrameScaleV36:c.frame_scale,calFrameXV37:c.frame_x??50,calFrameYV37:c.frame_y??50,calPlusXV36:c.plus_x,calPlusYV36:c.plus_y
  };
  Object.entries(vals).forEach(([id,v])=>{if($('#'+id))$('#'+id).value=Number(v)});
...
function updateFrameCalibratorPreviewV36(){
  ...
  // También se refleja en el perfil real si es el marco actualmente equipado.
  if((frameState.equipped||null)===id){
    [$('#homeAvatar'),$('#profileAvatar')].forEach(el=>applyCalibrationToElementV36(el,id,draft));
    const ownPlus=$('#profilePhotoMenuButton');
    if(ownPlus){ownPlus.style.left=`${draft.plus_x}%`;ownPlus.style.top=`${draft.plus_y}%`}
  }
}
function setupFrameCalibratorV36(){
  ...
  if(!select.dataset.boundV36){
    ...
    ['calInnerDiameterV38','calPhotoScaleV36','calPhotoXV36','calPhotoYV36','calFrameScaleV36','calFrameXV37','calFrameYV37','calPlusXV36','calPlusYV36'].forEach(id=>{
      $('#'+id)?.addEventListener('input',updateFrameCalibratorPreviewV36);
    });
  }
}
...
function startVisualLiveSyncV36(){
  ...
  stopVisualLiveV36=subscribeVisualLiveV36(session.user.id,{
    ...
    onCalibration:row=>{
      ...
      applyAllVisibleFrameCalibrationsV36();
      renderFrameGallery();
      // No pisamos sliders mientras el admin está moviéndolos.
      if(v35Flags?.is_test_admin && $('#frameCalibrationSelectV36')?.value!==row.frame_id){
...
function publicFramedAvatarV34(p,frameId='none'){
  ...
  return `<div class="public-avatar-frame avatar-frame-shell ${frameClass(frameId)} ${has?'has-photo':''}" data-has-photo="${has?'true':'false'}" data-frame-calibration-id="${esc(frameId)}" style="${calibrationStyleV36(frameId)}">
    <div class="avatar-photo-mask">
...
function populate(){
  ...
  if(avatar){
    ...
    applyCalibrationToElementV36(avatar,equippedFrameId);
  }
  ...
  if(profileAvatar&&profileAvatarImg&&profileAvatarFallback){
    ...
    applyCalibrationToElementV36(profileAvatar,equippedFrameId);
...
if($('#saveFrameCalibrationV36'))$('#saveFrameCalibrationV36').onclick=async()=>{
  ...
  try{
    ...
    applyAllVisibleFrameCalibrationsV36();
    renderFrameGallery();
    setStatus(st,'Calibración publicada en vivo para todos.','ok');
...
if($('#resetFrameCalibrationV36'))$('#resetFrameCalibrationV36').onclick=async()=>{
  ...
  try{
    ...
    applyAllVisibleFrameCalibrationsV36();
    renderFrameGallery();
    setStatus(st,'Valores originales restaurados.','ok');
  }catch(e){setStatus(st,e.message,'error')}
};

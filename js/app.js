...
import {initAiV61,refreshAiV61} from './v61_ai.js';
import {
...
function populate(){
  ...
  animateNumberV601($('#homeDoublesRating'),dob.rating,{suffix:' Elo',duration:560});
...
async function loadRanking(){
  ...
  try{
      ...
      return `<article class="rank-row rank-row-button v60-rank-row ${x.profile.id===session.user.id?'is-me':''}">
        <button class="v60-rank-profile" data-open-player="${p.id}" type="button">
          ...
          <div class="rank-name">
            ...
            <div class="v6011-rank-meta"><small>@${esc(p.username)}${x.profile.id===session.user.id?' · Vos':''}</small><span class="ranking-rank-chip ${rankCss(rank)}">${rank}</span></div>
        ...
        ${p.id===session.user.id?'':`<button class="v60-rank-challenge" data-quick-challenge="${p.id}" data-name="${esc(p.first_name)} ${esc(p.last_name)}" data-user="${esc(p.username)}" type="button" aria-label="Desafiar a ${esc(p.first_name)}"><span aria-hidden="true">⚔</span><b>Desafiar</b></button>`}
      </article>`;
    ...
    animateListV601(list,'.v60-rank-row',20);
...
async function loadHomeDashboard(){
  ...
  if($('#homeDoublesRating'))animateNumberV601($('#homeDoublesRating'),dob.rating,{suffix:' Elo',duration:520});

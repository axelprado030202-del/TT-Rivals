...
function effectivePhotoDiameterV38(c){
  const inner=Number(c?.inner_diameter??50);
  const outerScale=Number(c?.frame_scale??100)/100;
  const fill=Number(c?.photo_scale??92)/100;
  return Math.max(12,Math.min(95,inner*outerScale*fill));
}
...
function applyExactAvatarRendererV39(el,frameId,calibration=null){
  ...
  if(photo){
    ...
    photo.style.left=`${Number(c.photo_x??50)}%`;
    photo.style.top=`${Number(c.photo_y??50)}%`;
...
function framePreviewMarkupV36(frameId){
  ...
  return `<div class="frame-preview-v37" data-frame-calibration-id="${esc(frameId)}">
    <div class="frame-gallery-photo-v37" style="width:${effectivePhotoDiameterV38(c)}%;height:${effectivePhotoDiameterV38(c)}%;left:${Number(c.photo_x)}%;top:${Number(c.photo_y)}%">${has?`<img src="${esc(profile.profile_photo_url)}" alt="">`:`<span>${esc(initials)}</span>`}</div>
...
function publicFramedAvatarV34(p,frameId='none'){
  ...
  return `<div class="public-avatar-frame avatar-frame-shell avatar-renderer-v39 ${has?'has-photo':''}" data-has-photo="${has?'true':'false'}" data-frame-calibration-id="${esc(frameId)}">
    <div class="avatar-photo-mask" style="width:${effectivePhotoDiameterV38(c)}%;height:${effectivePhotoDiameterV38(c)}%;left:${Number(c.photo_x??50)}%;top:${Number(c.photo_y??50)}%;transform:translate(-50%,-50%)">
...
async function loadV35Flags(){
  ...
  $('#testAdminSectionV35')?.classList.toggle('hidden',!v35Flags.is_test_admin);
...
function activateTab(tab){
  ...
  if(tab==='settings'){populateSettingsUI();renderFrameGallery();loadV35Flags().then(()=>{renderFrameGallery();renderAchievements();setupFrameCalibratorV36();});}

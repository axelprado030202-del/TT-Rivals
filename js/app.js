...
(async()=>{
  try{
    ...
    if(recoveryHint){
      // La sesión de recovery es válida únicamente después de abrir el enlace
      // enviado por Supabase. Si ya fue procesada, mostramos directamente el
      // formulario para crear la nueva contraseña.
      const recoverySession=await getSession();
      if(recoverySession?.user){
        showPasswordRecoveryViewV53(recoverySession);
        return;
      }

      // Dejamos un breve margen a supabase-js para procesar el token/código
      // de recuperación y emitir PASSWORD_RECOVERY.
      await new Promise(resolve=>setTimeout(resolve,650));
      if(passwordRecoveryActiveV53)return;
    }

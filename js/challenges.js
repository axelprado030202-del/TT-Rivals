...
export async function getMyChallenges(userId,{force=false}={}){return challengesCacheV60.run(userId,async()=>{const {data:rows,error}=await supabase.from('challenges').select('*').or(`challenger_id.eq.${userId},challenged_id.eq.${userId}`).order('created_at',{ascending:false});if(error)throw error;if(!rows?.length)return[];const ids=[...new Set(rows.flatMap(r=>[r.challenger_id,r.challenged_id]))];const {data:profiles,error:e2}=await supabase.from('visible_profiles_v76').select('id,username,first_name,last_name,profile_photo_url').in('id',ids);if(e2)throw e2;const map=new Map((profiles||[]).map(p=>[p.id,p]));return rows.map(r=>({...r,challenger:map.get(r.challenger_id),challenged:map.get(r.challenged_id)})).filter(r=>r.challenger&&r.challenged);},{force});}
